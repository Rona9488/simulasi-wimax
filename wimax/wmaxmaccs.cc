...
void WMaxMacCS::handleMessage(inet::cMessage *msg) {
    if (dynamic_cast<WMaxMacTerminateAllConns*>(msg)) {
        Log(Notice) << "All connections terminated." << LogEnd;
        std::cout << "All connections terminated." << std::endl;
        csTable.clear();
        delete msg;
        return;
    }
    ...
    if (WMaxMacAddConn *addconn = dynamic_cast<WMaxMacAddConn*>(msg)) {
        ...
        std::cout << "cid = "<<addconn->getCid()<<"  dstAddr = "<<addconn->getDstAddr()<<"  macAddr = "<< addconn->getMacAddr() << std::endl;
    ...
    if (WMaxEvent_DelConn *delcon = dynamic_cast<WMaxEvent_DelConn*>(msg)) {
        ...
        for(it=csTable.begin(); it!=csTable.end(); it++) {
            if((it->cid == delcon->getCid())) {
                csTable.erase(it);
                std::cout << "Connection cid=" << delcon->getCid() << " removed." << std::endl;
        ...
        std::cout << "Unable to delete connection with cid=" << delcon->getCid() << "." << std::endl;
...
void WMaxMacCS::handleUlMessage(inet::cMessage *msg) {
    // reconstruction
    inet::cMessage *ipv6packet = check_and_cast<inet::cPacket *>(msg)->decapsulate();//MiM
    delete msg;
...
void WMaxMacCS::handleDlMessage(inet::cMessage *msg) {
    ...
    if (csTable.size() == 0){
        ...
        std::cout << "Unable to forward message(" << msg->getFullName() << "), no CS rules defined." << std::endl;
	...
	// on SS side, just use first connection available
        ...
        if (it->dstAddr == dstAddr){
            ...
            std::cout << "it->dstAddr == dstAddr ";
            dlMsgSend(msg, it->cid);
            return;
        }
        if (dstAddr.isMulticast()) {
            std::cout << "dstAddr.isMulticast()";
            mcastReceiversCnt++;
            dlMsgSend((inet::cMessage*)msg->dup(), it->cid);
        }
//============= Adam 14-09-2011 =====================        
...
void WMaxMacCS::dlMsgSend(inet::cMessage * msg, int cid)
{
    std::cout << " -> sending DL msg" << std::endl;
    list<WMaxMacCSRule>::iterator it;
    
  /*cMessage*/ inet::cPacket *wmaxmacmsg = new inet::cPacket/*Message*/(msg->getFullName());//MiM
    wmaxmacmsg->encapsulate(check_and_cast<inet::cPacket *>(msg)); //MiM
