...
static int GetCidFromMsg(inet::cMessage * msg)
{
    WMaxMacHeader * hdr = dynamic_cast<WMaxMacHeader*>(msg->getControlInfo());
    if (!hdr) {
        throw inet::cException("Unable to find MacHeader in message %s\n", msg->getFullName());
    }
    return hdr->cid;
}
...
inet::simtime_t /*double*/ WMaxCtrlSS::sendMsg(inet::cMessage * msg, std::string paramName, const std::string &gateName, int cid, double extraDelay)
{
    double delay = 0;
 ...
 * @param msg 
 */
void WMaxCtrlSS::handleMessage(inet::cMessage *msg)

{
    ...
    // messages
    if (dynamic_cast<WMaxMsgRngRspMsg*>(msg)) {
	onEvent(EVENT_RNG_RSP_RECEIVED, msg);
	delete msg;
	return;
    }
    if (dynamic_cast<WMaxMsgSbcRspMsg*>(msg)) {
...
// wstawienie NCoA jako CoA
        ...
        if( this->getParentModule()->getParentModule()->par("FMIP") ){
            ...
            if( ie -> ipv6Data() -> getNumAddresses() < 3 ){ // jesli jest tylko link-local i HoA
                ...
                std::cout << "Handover End removing CoA = " << removeCoA.str()
                   << " new CoA = " << ift2->getHandover_NCoA().str() << std::endl;
                ...
                ie->ipv6Data()->assignAddress( ift2->getHandover_NCoA(), isLinkLocalTentative, inet::simTime(), inet::simTime()+100, false);   // poprawic, nie wiem czy dobrze zrobione
            }
        }
        Handover = 1;
//============= Adam, end  11-09-2011==================
...
FsmStateType WMaxCtrlSS::onEventState_WaitForDlmap(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    switch (e) {
    case EVENT_DLMAP:
...
// wait for UCD state
FsmStateType WMaxCtrlSS::onEventState_WaitforUcd(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    switch (e) {
    case EVENT_UCD:
...
/* *** STATE: WAIT FOR CDMA ************************************************************ */
FsmStateType WMaxCtrlSS::onEventState_WaitForCdma(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
...
FsmStateType WMaxCtrlSS::onEvent_CdmaCode(inet::cMessage *msg)
{
...
//     cModule *SS = ss->getParentModule();
    ...
    if (!SSInfo)
        throw inet::cException("Unable to obtain ssInfo submodule in SS[%d]\n", ss->SS->getIndex());
    ssInfo *ssinfo = dynamic_cast<ssInfo*>(SSInfo);

    WMaxRngReq rngReq;
    rngReq.macAddr = ssinfo->info.macAddr;

    WMaxMsgRngReqMsg * rng = new WMaxMsgRngReqMsg();
    rng->setName("RNG-REQ");
...
FsmStateType WMaxCtrlSS::onEventState_WaitForRngRsp(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    WMaxCtrlSS * ss = dynamic_cast<WMaxCtrlSS*>(fsm);
    ...
    case EVENT_RNG_RSP_RECEIVED: 
      {
        WMaxMsgRngRspMsg * rng = dynamic_cast<WMaxMsgRngRspMsg*>(msg);
...
FsmStateType WMaxCtrlSS::onEnterState_SendSbcReq(Fsm * fsm)
{
    ...
    WMaxMsgSbcReqMsg * req = new WMaxMsgSbcReqMsg();
    req->setName("SBC-REQ");
...
FsmStateType WMaxCtrlSS::onEventState_WaitForSbcRsp(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    WMaxCtrlSS * ss = dynamic_cast<WMaxCtrlSS*>(fsm);
    switch (e) {
...
FsmStateType WMaxCtrlSS::onEventState_WaitForSaTekChallange(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    switch (e) {
...
// wait for SA-TEK-RSP
FsmStateType WMaxCtrlSS::onEventState_WaitForSaTekRsp(Fsm *fsm, FsmEventType e, inet::cMessage *msg)
{
    switch (e) {
...
FsmStateType WMaxCtrlSS::onEventState_WaitForRegRsp(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    switch (e) {
...
FsmStateType WMaxCtrlSS::onEnterState_SvcFlowCreation(Fsm * fsm) {
    ...
    //MiM ........................
    inet::cModuleType *mtype = inet::cModuleType::get("WMaxFlowSS");
...
FsmStateType WMaxCtrlSS::onEventState_SvcFlowCreation(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    WMaxCtrlSS * ss = dynamic_cast<WMaxCtrlSS*>(fsm);
    switch (e) {
...
FsmStateType WMaxCtrlSS::onEnterState_Operational(Fsm * fsm)
{
    ...
    //double x, y;
    inet::simtime_t x, y;//MiM

    switch (ss->neType) {
    case WMAX_CTRL_NETWORK_REENTRY:
      x = inet::simTime() - ss->hoReentryTimestamp;
      ss->hoReentryCompleteTimestamp=inet::simTime();
      ...
      SLog(fsm, Notice) << "Network reentry complete: " << x << "secs (" 
                         << ss->hoReentryTimestamp << "-" << inet::simTime() << ")." << LogEnd;
...
FsmStateType WMaxCtrlSS::onEventState_Operational(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    WMaxCtrlSS * ss = dynamic_cast<WMaxCtrlSS *>(fsm);
...
//============= Adam 10-09-2011 =====================
void WMaxCtrlSS::HandoverAck(inet::cMessage *msg)
{
    Enter_Method("HandoverAck");
    take(msg);
    onEvent(EVENT_HANDOVER_ACK_RECEIVED, msg);
}
FsmStateType WMaxCtrlSS::onEventState_WaitForHandoverAck(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    // WMaxCtrlSS * ss = dynamic_cast<WMaxCtrlSS *>(fsm);
...
FsmStateType WMaxCtrlSS::onEventState_WaitForMobScnRsp(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
WMaxCtrlSS * ss = dynamic_cast<WMaxCtrlSS*>(fsm);
int actBS, nearestBS;
...
{
    // Dynamic cast to WMaxCtrlSS to access SS-specific functionality
    WMaxCtrlSS *ctrlSS = dynamic_cast<WMaxCtrlSS *>(fsm);

    // Get the current Base Station (BS) module
    inet::cModule *BS = ctrlSS->SS->gate("out")->getNextGate()->getOwnerModule();
    int actBS = BS->getIndex(); // Current BS index

    // Check if the SS is mobile
    int isMobile = ctrlSS->SS->par("wmaxIsMobile");

    // Determine the target BS based on mobility model
    int numBS = BS->getParentModule()->gateSize("out"); // Total number of BSs
    if (isMobile == 1) // Mobility model 1: Time-based handover
    {
        ctrlSS->hoInfo->wmax.nextBS = (actBS + 1) % numBS; // Circular next BS
    }
    else if (isMobile == 3) // Mobility model 3: Time-based, random target
    {
        do {
            ctrlSS->hoInfo->wmax.nextBS = std::rand() % numBS; // Random BS selection
        } while (ctrlSS->hoInfo->wmax.nextBS == actBS); // Avoid selecting the current BS
    }

    // Set the handover start timestamp
    ctrlSS->hoStartTimestamp = inet::simTime();

    // Access the SS submodule containing additional information
    ssInfo *ssinfo = dynamic_cast<ssInfo *>(ctrlSS->SS->getSubmodule("ssInfo"));

    // Create and configure the MSHO-REQ message
    WMaxMsgMSHOREQ *mshoReq = new WMaxMsgMSHOREQ("MSHO-REQ");
    mshoReq->setName("MSHO-REQ");

    // Log the handover initiation
    SLog(fsm, Notice) << "Starting handover to BS[" << ctrlSS->hoInfo->wmax.nextBS
                      << "], sending MSHO-REQ message." << LogEnd;

    // Send the MSHO-REQ message
    ctrlSS->sendMsg(mshoReq, "", "macOut", ssinfo->info.basicCid);

    // Notify the upper layer about the handover event
    ctrlSS->mihNotify(MIH_EVENT_HANDOVER_START, ctrlSS->hoInfo->wmax.nextBS);
...
void WMaxCtrlSS::mihNotify(MihInfo_t notifyType, int data)
{
    ...
    switch (notifyType) {
      case MIH_EVENT_ENTRY_START:
        x = (inet::cMessage*) new MihEvent_EntryStart();
        str = "Network Entry Start";
        break;
      case MIH_EVENT_ENTRY_END:
        x = (inet::cMessage*) new MihEvent_EntryEnd();
	...
	hoStart->setTargetBS(data);
        x = (inet::cMessage*) hoStart;
	char buf[16];
	sprintf(buf, "%d", data);
        str = "Handover Start (target BS[" + string(buf) + "]";
        InactivityL3StartTime = inet::simTime();
        break;
      }
      case MIH_EVENT_HANDOVER_END:
        x = (inet::cMessage*) new MihEvent_HandoverEnd();
        str = "Handover End";
        break;
      case MIH_EVENT_REENTRY_START:
        x = (inet::cMessage*) new MihEvent_ReentryStart();
        str = "Network Reentry Start";
        break;
      case MIH_EVENT_REENTRY_END:
        x = (inet::cMessage*) new MihEvent_ReentryEnd();
        ...
        if( Handover == 1){
            ...
            L3Ready = inet::simTime();
            InactivityL3Duration.record(inet::simTime() - InactivityL3StartTime );
        }
        Handover = 0;
        break;
    }
...
FsmStateType WMaxCtrlSS::onEventState_WaitForBshoRsp(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    WMaxCtrlSS * ctrlSS = dynamic_cast<WMaxCtrlSS*>(fsm);
    switch (e) {
...
FsmStateType WMaxCtrlSS::onEventState_WaitForL3Detach(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    switch (e) {
...
FsmStateType WMaxCtrlSS::onEventState_SendHoInd(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
...
{   WMaxCtrlSS *ss = dynamic_cast<WMaxCtrlSS*>(fsm);
    ...
    ss->connectNextBS();
    ss->hoReentryTimestamp = inet::simTime();
...
FsmStateType WMaxCtrlSS::onEventState_WaitForAnonRngRsp(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    switch (e) {
    case EVENT_RNG_RSP_RECEIVED:
	return STATE_SEND_RNG_REQ;
...
FsmStateType WMaxCtrlSS::onEventState_PowerDown(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
    WMaxCtrlSS * ss = dynamic_cast<WMaxCtrlSS*>(fsm);
...
//     cModule *SS = getParentModule();
    ...
    if (x> maxBS) {
        throw inet::cException("Unable to connect to BS[%d]: there are only %d BS(es)\n",
		  x, maxBS);
    }
    
    cModule * targetBS = physim->getSubmodule("BS",x);
    if (!targetBS) {
        throw inet::cException("Unable to find BS[%d]\n", x);
    } 
    ...
    if (!radio)
        throw inet::cException("Unable to obtain radio submodule in BS[%d]\n",
		  x);
...
double WMaxCtrlBS::sendMsg(inet::cMessage * msg, const char * paramName, const char * gateName, int cid)
{
    char buf[80];
...
SSInfo_t * WMaxCtrlBS::getSS(uint16_t basicCid, string reason)
{
  ...
  throw inet::cException("Unable to find SS with cid=%d while %s\n", basicCid, reason.c_str());
...
void WMaxCtrlBS::deleteSS(uint16_t basicCid)
{
  ...
  throw inet::cException("Unable to delete SS with cid=%d. SS not found.\n", basicCid);
}

void WMaxCtrlBS::handleMessage(inet::cMessage *msg)
{
    if (dynamic_cast<WMaxMsgRngReqMsg*>(msg)) {
      int basicCid;
      if (GetCidFromMsg(msg) != WMAX_CID_RANGING )
        throw inet::cException("Received RNG-REQ on non-ranging connection (cid=%d)", GetCidFromMsg(msg));

      WMaxMsgRngReqMsg * req = dynamic_cast<WMaxMsgRngReqMsg*>(msg);
      WMaxRngReq rngReq = req->getRngReq();
      WMaxMsgRngRspMsg * rsp = new WMaxMsgRngRspMsg("RNG-RSP (initial rng)");
    ...
    if (dynamic_cast<WMaxMsgSbcReqMsg*>(msg)) {
        WMaxMsgSbcRspMsg * rsp = new WMaxMsgSbcRspMsg("SBC-RSP");
    ...
    if (dynamic_cast<WMaxMsgCDMA*>(msg)) {
        ...
        switch (cdma->getPurpose()) {
          case WMAX_CDMA_PURPOSE_HO_RNG:
            {
              ...
              WMaxMsgRngRspMsg * rsp = new WMaxMsgRngRspMsg();
          ...
          default:
            ...
            throw inet::cException("That type of CDMA code is not supported yet (purpose=%d).", cdma->getPurpose());
            break;
        }
...
void WMaxFlowSS::handleMessage(inet::cMessage *msg) {
...
// Start state
FsmStateType WMaxFlowSS::onEventState_Start(Fsm * fsm, FsmEventType e, inet::cMessage * msg){
...
FsmStateType WMaxFlowSS::onEventState_WaitingDsxRvd(Fsm * fsm, FsmEventType e, inet::cMessage * msg) {
    switch (e) {
...
FsmStateType WMaxFlowSS::onEventState_WaitingDsaRsp(Fsm * fsm, FsmEventType e, inet::cMessage * msg) {
    WMaxMsgDsaRsp *dsarsp = dynamic_cast<WMaxMsgDsaRsp*>(msg);
    WMaxFlowSS *flow = dynamic_cast<WMaxFlowSS*>(fsm);
...
// Operational state
FsmStateType WMaxFlowSS::onEventState_Operational(Fsm * fsm, FsmEventType e, inet::cMessage * msg) {
    switch (e) {
...
FsmStateType WMaxFlowSS::onEventState_Disabled(Fsm * fsm, FsmEventType e, inet::cMessage *msg)
{
...
WMaxEvent_FlowCreationStart * WMaxCtrlSS::createNewFlowEvent()
{
    ...
    default: throw inet::cException("Invalid connection type for %s: %d (allowed: 2=BE, 6=UGS)", getFullName(), type);
    }
