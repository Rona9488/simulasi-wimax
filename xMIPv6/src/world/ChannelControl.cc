...
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\util\FWMath.h"
#include <cassert>


#define coreEV (coreDebug ? std::cout << "ModuleName::ChannelAccess: " << __FILE__ << ":" << __LINE__ : std::cout)
...
// In ChannelControl.cc

ChannelControl *ChannelControl::get()
{
    // Mendapatkan simulasi global
    omnetpp::cSimulation *sim = omnetpp::getSimulation();

    // Mencari modul ChannelControl dengan path yang tepat
    ChannelControl *cc = dynamic_cast<ChannelControl *>(sim->getModuleByPath("channelcontrol"));
    if (!cc)
        cc = dynamic_cast<ChannelControl *>(sim->getModuleByPath("channelControl"));

    // Jika tidak ditemukan, lemparkan error
    if (!cc)
        throw omnetpp::cRuntimeError("Could not find ChannelControl module");
 ...
 */
void ChannelControl::updateDisplayString(inet::cModule *playgroundMod)
{
    inet::cDisplayString& d = playgroundMod->getDisplayString();
...
double ChannelControl::calcInterfDist()
{
    ...
    coreEV << "max interference distance:" << interfDistance << std::endl;

    return interfDistance;
}

ChannelControl::HostRef ChannelControl::registerHost(inet::cModule *host, const Coord& initialPos, inet::cGate *radioInGate)
{
    Enter_Method_Silent();
...
ChannelControl::HostRef ChannelControl::lookupHost(inet::cModule *host)
{
    Enter_Method_Silent();
...
void ChannelControl::addOngoingTransmission(HostRef h, AirFrame *frame)
{
    ...
    if (inet::simTime() - lastOngoingTransmissionsUpdate > TRANSMISSION_PURGE_INTERVAL)
    {
        purgeOngoingTransmissions();
        lastOngoingTransmissionsUpdate = inet::simTime();
    }
...
void ChannelControl::purgeOngoingTransmissions()
{
    for (int i = 0; i < numChannels; i++)
    {
        for (TransmissionList::iterator it = transmissions[i].begin(); it != transmissions[i].end();)
        {
            ...
            if (frame->getTimestamp() + frame->getDuration() + TRANSMISSION_PURGE_INTERVAL < inet::simTime())
            {
                delete frame;
...
void ChannelControl::sendToChannel(cSimpleModule *srcRadioMod, HostRef srcHost, AirFrame *airFrame)
{
    ...
    for (int i=0; i<n; i++)
    {
        ...
        if (h->channel == channel)
        {
            ...
            inet::simtime_t delay = srcHost->pos.distance(h->pos) / LIGHT_SPEED;
