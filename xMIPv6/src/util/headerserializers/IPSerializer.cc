...
int IPSerializer::serialize(const IPDatagram *dgram, unsigned char *buf, unsigned int bufsize)
{
    ...
    if (dgram->getHeaderLength() > IP_HEADER_BYTES)
        std::cout << "Serializing an IP packet with options. Dropping the options.\n";

    packetLength = IP_HEADER_BYTES;

    inet::cMessage *encapPacket = check_and_cast<inet::cMessage *>(const_cast<IPDatagram *>(dgram)->getObject("encapsulatedMsg"));
    switch (dgram->getTransportProtocol())
    {
      ...
      case IP_PROT_TCP:        //I.R.
        ...
        break;
      default:
        throw inet::cException("IPSerializer: cannot serialize protocol %d", dgram->getTransportProtocol());
    }
...
void IPSerializer::parse(const unsigned char *buf, unsigned int bufsize, IPDatagram *dest)
{
    ...
    if (headerLength > (unsigned int)IP_HEADER_BYTES)
        std::cout << "Handling an captured IP packet with options. Dropping the options.\n";
    if (totalLength > bufsize)
        std::cout << "Can not handle IP packet of total length " << totalLength << "(captured only " << bufsize << " bytes).\n";
    dest->setByteLength(IP_HEADER_BYTES);

    inet::cPacket *encapPacket = NULL;
    switch (dest->getTransportProtocol())
    {
      ...
      case IP_PROT_TCP:
        ...
        break;
      default:
        throw inet::cException("IPSerializer: cannot serialize protocol %d", dest->getTransportProtocol());
    }
