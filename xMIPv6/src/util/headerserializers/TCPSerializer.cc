        ...
        const IPvXAddress &srcIp, const IPvXAddress &destIp)
{
    ...
    //sum += srcip;
    sum += htons(TCPIPchecksum::_checksum(srcIp.words(), sizeof(uint32_t)*srcIp.wordCount()));

    //sum += destip;
    sum += htons(TCPIPchecksum::_checksum(destIp.words(), sizeof(uint32_t)*destIp.wordCount()));
