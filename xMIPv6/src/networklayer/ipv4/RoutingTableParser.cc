...
int RoutingTableParser::readRoutingTableFromFile(const char *filename)
{
    ...
    fp = fopen(filename, "r");
    if (fp == NULL)
        throw inet::cException("Error opening routing table file `%s'", filename);
...
void RoutingTableParser::parseInterfaces(char *ifconfigFile)
{
    ...
    while(ifconfigFile[charpointer] != '\0')
    {
        ...
        if (streq(ifconfigFile + charpointer, "name:")) {
            ...
            if (!ie)
                throw inet::cException("Error in routing file: interface name `%s' not registered by any L2 module", name);
            continue;
        }
...
void RoutingTableParser::parseMulticastGroups(char *groupStr, InterfaceEntry *itf)
{
    ...
    inet::cStringTokenizer tokenizer(groupStr,":");
    const char *token;
    ...
    while (routeFile[pos] != '\0')
    {
        ...
        if (strcmp(str, "default:"))
        {
            ...
            if (!IPAddress::isWellFormed(str))
                throw inet::cException("Syntax error in routing file: `%s' on 1st column should be `default:' or a valid IP address", str);
        ...
        {
            if (!IPAddress::isWellFormed(str))
                throw inet::cException("Syntax error in routing file: `%s' on 2nd column should be `*' or a valid IP address", str);
        ...
        skipBlanks(routeFile, pos);
        if (!IPAddress::isWellFormed(str))
            throw inet::cException("Syntax error in routing file: `%s' on 3rd column should be a valid IP address", str);
        ...
        for (int i = 0; str[i]; i++)
        {
            if (str[i] == 'H') {
                ...
                throw inet::cException("Syntax error in routing file: 4th column should be `G' or `H' not `%s'", str);
            }
        }
        ...
        int metric = atoi(str);
        if (metric==0 && str[0]!='0')
            throw inet::cException("Syntax error in routing file: 5th column should be numeric not `%s'", str);
        e->setMetric(metric);

        // 6th entry: interface
        inet::opp_string interfaceName;
        ...
        if (!ie)
            throw inet::cException("Syntax error in routing file: 6th column: `%s' is not an existing interface", interfaceName.c_str());
        e->setInterface(ie);
