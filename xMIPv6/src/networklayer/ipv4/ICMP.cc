...
void ICMP::handleMessage(inet::cMessage *msg)
{
    inet::cGate *arrivalGate = msg->getArrivalGate();
...
void ICMP::sendErrorMessage(IPDatagram *origDatagram, ICMPType type, ICMPCode code)
{
    ...
    {
        ICMPMessage *recICMPMsg = check_and_cast<ICMPMessage *>(origDatagram->getObject("encapsulatedMessage"));
    ...
    std::cout << "sending ICMP error " << msgname << std::endl;
...
void ICMP::sendErrorMessage(inet::cPacket *transportPacket, IPControlInfo *ctrl, ICMPType type, ICMPCode code)
{
...
void ICMP::processICMPMessage(ICMPMessage *icmpmsg)
{
    switch (icmpmsg->getType())
    {
        ...
        case ICMP_TIMESTAMP_REPLY:
            ...
            break;
        default:
            throw inet::cException("Unknown ICMP type %d", icmpmsg->getType());
    }
}
...
void ICMP::sendEchoRequest(inet::cPacket *msg)
{
