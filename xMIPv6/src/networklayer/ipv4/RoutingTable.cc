...
void RoutingTable::handleMessage(inet::cMessage *msg)
{
    throw inet::cException("This module doesn't process messages");
}

void RoutingTable::receiveChangeNotification(int category, const inet::cObject *details)
{
    if (inet::cSimulation::getActiveSimulation()->getContextType() == inet::CTX_INITIALIZE)
    ...
    else if (category==NF_INTERFACE_DELETED)
    {
        ...
        const InterfaceEntry *entry = omnetpp::check_and_cast<const InterfaceEntry *>(details);
        deleteInterfaceRoutes(const_cast<InterfaceEntry *>(entry));
...
void RoutingTable::printRoutingTable() const
{
    ...
    std::cout << "Destination       Gateway           Netmask           Iface Info\n";

    for (int i=0; i<getNumRoutes(); i++)
        std::cout << getRoute(i)->detailedInfo() << "\n";
    std::cout << "\n";
}
