...
void IP::endService(inet::cPacket *msg)
{
    ...
    if (getSimulation()->getActiveEnvir()->isGUI())
        updateDisplayString();
}

InterfaceEntry *IP::getSourceInterfaceFrom(inet::cPacket *msg)
{
...
void IP::handleReceivedICMP(ICMPMessage *msg)
{
    switch (msg->getType())
    {
        ...
        case ICMP_PARAMETER_PROBLEM: {
            ...
            // Explicitly cast the encapsulated message to IPDatagram
            IPDatagram *bogusPacket = dynamic_cast<IPDatagram *>(msg->getObject("encapsulatedMessage"));

            if (bogusPacket) {
                // Successfully cast to IPDatagram
                int protocol = bogusPacket->getTransportProtocol();
                int gateindex = mapping.getOutputGateForProtocol(protocol);
                send(msg, "transportOut", gateindex);
            } else {
                // Error: Encapsulated message is not of type IPDatagram
                EV_ERROR << "Error: Encapsulated message is not of type IPDatagram." << std::endl;
                delete msg;
            }
            break;
        }
        default: {
            // For other ICMP types (like ICMP_ECHO_REQUEST, ICMP_ECHO_REPLY, etc.)
...
void IP::handleMessageFromHL(inet::cPacket *msg)
{
...
{
    IPAddress destAddr = datagram->getDestAddress();
    std::cout << "Routing multicast datagram `" << datagram->getName() << "' with dest=" << destAddr << "\n";
    ...
    if (destIE!=NULL)
    {
        ...
        std::cout << "multicast packet explicitly routed via output interface " << destIE->getName() << std::endl;
...
void IP::reassembleAndDeliver(IPDatagram *datagram)
{
    ...
    {
        std::cout << "Datagram fragment: offset=" << datagram->getFragmentOffset()
           << ", MORE=" << (datagram->getMoreFragments() ? "true" : "false") << ".\n";

        // erase timed out fragments in fragmentation buffer; check every 10 seconds max
        if (inet::simTime() >= lastCheckTime + 10)
        {
            lastCheckTime = inet::simTime();
            fragbuf.purgeStaleFragments(inet::simTime()-fragmentTimeoutTime);
        }

        datagram = fragbuf.addFragment(datagram, inet::simTime());
        if (!datagram)
        {
            std::cout << "No complete datagram yet.\n";
            return;
        }
        std::cout << "This fragment completes the datagram.\n";
    }

    // decapsulate and send on appropriate output gate
    int protocol = datagram->getTransportProtocol();
    inet::cPacket *packet = decapsulateIP(datagram);
...
inet::cPacket *IP::decapsulateIP(IPDatagram *datagram)
{
...
void IP::fragmentAndSend(IPDatagram *datagram, InterfaceEntry *ie, IPAddress nextHopAddr)
{
    ...
    std::cout << "Breaking datagram into " << noOfFragments << " fragments\n";
...
IPDatagram *IP::encapsulate(inet::cPacket *transportPacket, InterfaceEntry *&destIE)
{
...
IPDatagram *IP::encapsulate(inet::cPacket *transportPacket, InterfaceEntry *&destIE, IPControlInfo *controlInfo)
{
    ...
    if (!src.isUnspecified())
    {
        ...
        if (rt->getInterfaceByAddress(src)==NULL)
            throw inet::cException("Wrong source address %s in (%s)%s: no interface with such address",
