...
bool REDQueue::enqueue(inet::cMessage *msg)
{
    //"
    ...
    else
    {
        ...
        double m = SIMTIME_DBL(inet::simTime()-q_time) * pkrate;
        avg = pow(1-wq, m) * avg;
    }
    ...
    if (minth<=avg && avg<maxth)
    {
        ...
        if (dblrand() < pa)
        {
            std::cout << "Random early packet drop (avg queue len=" << avg << ", pa=" << pa << ")\n";
            mark = true;
            count = 0;
            numEarlyDrops++;
        }
    ...
    {
        std::cout << "Avg queue len " << avg << " >= maxth, dropping packet.\n";
        mark = true;
        count = 0;
    }
    else
...
inet::cMessage *REDQueue::dequeue()
{
    if (queue.empty())
        return NULL;
...
void REDQueue::sendOut(inet::cMessage *msg)
{
    send(msg, outGate);
}
