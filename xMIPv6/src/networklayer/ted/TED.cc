...
void TED::initialize(int stage)
{
    ...
    for (int i = 0; i < ift->getNumInterfaces(); i++)
    {
        ...
        inet::cGate *g = getParentModule()->gate(ie->getNodeOutputGateId());
        ASSERT(g);
        ...
        std::cout << "metric set to=" << entry.metric << std::endl;

        entry.sourceId = routerId.getInt();
        entry.messageId = ++maxMessageId;
        entry.timestamp = inet::simTime();
...
void TED::handleMessage(inet::cMessage * msg)
{
    ASSERT(false);
}
...
{
    std::cout << "rebuilding routing table at " << routerId << std::endl;
...
//  for (unsigned int i = 0; i < V.size(); i++)
//  {
//      std::cout << "V[" << i << "].node=" << V[i].node << std::endl;
//      std::cout << "V[" << i << "].parent=" << V[i].parent << std::endl;
//      std::cout << "V[" << i << "].dist=" << V[i].dist << std::endl;
//  }
    ...
    for (unsigned int i = 0; i < V.size(); i++)
    {
        ...
        std::cout << "  inserting route: host=" << entry->getHost() << " interface=" << entry->getInterfaceName() << " nexthop=" << entry->getGateway() << "\n";
    ...
    for (unsigned int i = 0; i < interfaceAddrs.size(); i++)
    {
        ...
        std::cout << "  inserting route: local=" << interfaceAddrs[i] << " peer=" << entry->getHost() << " interface=" << entry->getInterfaceName() << "\n";
