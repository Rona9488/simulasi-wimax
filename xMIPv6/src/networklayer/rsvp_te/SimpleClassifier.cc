...
void SimpleClassifier::handleMessage(inet::cMessage *)
{
    ASSERT(false);
}
...
bool SimpleClassifier::lookupLabel(IPDatagram *ipdatagram, LabelOpVector& outLabel, std::string& outInterface, int& color)
{
    ...
    for (it = bindings.begin(); it != bindings.end(); it++)
    {
        ...
        std::cout << "packet belongs to fecid=" << it->id << std::endl;
...
void SimpleClassifier::processCommand(const inet::cXMLElement& node)
{
...
void SimpleClassifier::readTableFromXML(const inet::cXMLElement *fectable)
{
    ASSERT(fectable);
    ASSERT(!strcmp(fectable->getTagName(), "fectable"));
    checkTags(fectable, "fecentry");
    inet::cXMLElementList list = fectable->getChildrenByTagName("fecentry");
    for (inet::cXMLElementList::iterator it=list.begin(); it != list.end(); it++)
        readItemFromXML(*it);
}

void SimpleClassifier::readItemFromXML(const inet::cXMLElement *fec)
{
    ASSERT(fec);
    ...
    if (getUniqueChildIfExists(fec, "label"))
    {
        ...
        std::cout << "binding to a given label" << std::endl;
    ...
    else if (getUniqueChildIfExists(fec, "lspid"))
    {
        ...
        std::cout << "binding to a given path" << std::endl;
