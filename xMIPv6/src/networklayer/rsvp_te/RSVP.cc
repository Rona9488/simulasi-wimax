...
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\util\common.h"
#include "Utils.h"
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\util\XMLUtils.h"
...
void RSVP::createPath(const SessionObj_t& session, const SenderTemplateObj_t& sender)
{
    ...
    {
        std::cout << "path (PSB) already exists, doing nothing" << std::endl;
        return;
    }
    ...
    if (sit == traffic.end())
    {
        std::cout << "session not found in traffic database, path won't be created" << std::endl;
        return;
    }
    ...
    {
        std::cout << "path doesn't belong to this session according to our database, doing nothing" << std::endl;
        return;
    }
    ...
    {
        std::cout << "ingress PSB couln't be created" << std::endl;
        ...
        if (!pit->permanent)
        {
            std::cout << "removing path from traffic database" << std::endl;

            sit->paths.erase(pit--);
        }
        else
        {
            std::cout << "path is permanent, we will try again later" << std::endl;
...
void RSVP::readTrafficFromXML(const inet::cXMLElement* traffic)
{
    ASSERT(traffic);
    ASSERT(!strcmp(traffic->getTagName(), "sessions"));
    checkTags(traffic, "session");
    inet::cXMLElementList list = traffic->getChildrenByTagName("session");
    for (inet::cXMLElementList::iterator it=list.begin(); it != list.end(); it++)
        readTrafficSessionFromXML(*it);
}

EroVector RSVP::readTrafficRouteFromXML(const inet::cXMLElement *route)
{
    checkTags(route, "node lnode");

    EroVector ERO;

    for (inet::cXMLElement *hop = route->getFirstChild(); hop; hop = hop->getNextSibling())
    {
        EroObj_t h;
...
void RSVP::readTrafficSessionFromXML(const inet::cXMLElement *session)
{
    ...
    const inet::cXMLElement *paths = getUniqueChild(session, "paths");
    checkTags(paths, "path");

    inet::cXMLElementList list = paths->getChildrenByTagName("path");
    for (inet::cXMLElementList::iterator it=list.begin(); it != list.end(); it++)
    {
        inet::cXMLElement *path = *it;
        ...
        if (merge)
        {
            ...
            if (pit != sit->paths.end())
            {
                std::cout << "path " << lspid << " already exists in this session, doing nothing" << std::endl;
                continue;
            }
        }
        else
        ...
        if (strlen(str))
        {
            inet::cModule *mod = getSimulation()->getModuleByPath(str);
        ...
        const inet::cXMLElement *route = getUniqueChildIfExists(path, "route");
        if (route)
            newPath.ERO = readTrafficRouteFromXML(route);

        if (merge)
        {
            std::cout << "adding new path into an existing session" << std::endl;

            sit->paths.push_back(newPath);
        }
        else
        {
            std::cout << "adding new path into new session" << std::endl;
    ...
    if (!merge)
    {
        std::cout << "adding new session into database" << std::endl;
...
void RSVP::startHello(IPAddress peer, inet::simtime_t delay)
{
    std::cout << "scheduling hello start in " << delay << " seconds" << std::endl;
    ...
    scheduleAt(inet::simTime() + delay, h->timer);
}

void RSVP::sendPathNotify(int handler, const SessionObj_t& session, const SenderTemplateObj_t& sender, int status, inet::simtime_t delay)
{
    if (handler < 0)
        return; // handler not specified

    inet::cModule *mod = getSimulation()->getModule(handler);
...
void RSVP::processHELLO_TIMEOUT(HelloTimeoutMsg* msg)
{
    ...
    std::cout << "hello timeout, considering " << peer << " failed" << std::endl;
...
void RSVP::processHELLO_TIMER(HelloTimerMsg* msg)
{
    ...
    scheduleAt(inet::simTime() + helloInterval, msg);
}
...
{
    std::cout << "RSB TIMEOUT RSB " << msg->getId() << std::endl;
...
bool RSVP::doCACCheck(const SessionObj_t& session, const SenderTspecObj_t& tspec, IPAddress OI)
{
    ...
    std::cout << "CACCheck: link=" << OI <<
        " requested=" << tspec.req_bandwidth <<
        " shared=" << sharedBW <<
        " available (immediately)=" << tedmod->ted[k].UnResvBandwidth[7] <<
        " available (preemptible)=" << tedmod->ted[k].UnResvBandwidth[session.setupPri] << std::endl;
...
{
    std::cout << "refresh path (PSB " << psbEle->id << ")" << std::endl;
...
{
    std::cout << "refresh reservation (RSB " << rsbEle->id << ")" << std::endl;
...
{
    std::cout << "refresh reservation (RSB " << rsbEle->id << ") PHOP " << PHOP << std::endl;
...
void RSVP::preempt(IPAddress OI, int priority, double bandwidth)
{
    ASSERT(tedmod->isLocalAddress(OI));
...
bool RSVP::allocateResource(IPAddress OI, const SessionObj_t& session, double bandwidth)
{
    if (OI.isUnspecified())
        return true;
...
{
    std::cout << "commit reservation (RSB " << rsb->id << ")" << std::endl;

    // allocate bandwidth as needed

    std::cout << "currently allocated: " << rsb->Flowspec_Object << std::endl;
    ...
    while(true)
    {
        ...
        std::cout << "currently required: " << req << std::endl;
        ...
        if (needed != 0.0)
        {
            if (allocateResource(rsb->OI, rsb->Session_Object, needed))
            {
                ...
                std::cout << "additional bandwidth of " << needed << " allocated sucessfully" << std::endl;
            ...
            else
            {
                ...
                std::cout << "not enough bandwidth to accommodate this RSB" << std::endl;
                ...
                std::cout << "removing filter lspid=" << lspid << " (max. flow)" << std::endl;
    ...
    for (unsigned int i = 0; i < rsb->FlowDescriptor.size(); i++)
    {
        ...
        std::cout << "processing lspid=" << lspid << std::endl;
        ...
        std::cout << "installing label for " << lspid << " outLabel=" << outLabel <<
            " outInterface=" << outInterface << std::endl;
...
RSVP::ResvStateBlock_t* RSVP::createRSB(RSVPResvMsg *msg)
{
    ...
    std::cout << "created new RSB " << rsb->id << std::endl;
...
void RSVP::updateRSB(ResvStateBlock_t* rsb, RSVPResvMsg *msg)
{
    ...
    for (unsigned int k = 0; k < msg->getFlowDescriptor().size(); k++)
    {
        ...
        for (m = 0; m < rsb->FlowDescriptor.size(); m++)
        {
            ...
            {
                // sender found
                std::cout << "sender (lspid=" << flow.Filter_Spec_Object.Lsp_Id << ") found in RSB" << std::endl;

                if (rsb->FlowDescriptor[m].label != flow.label)
                {
                    std::cout << "label modified (new label="  << flow.label << ")" << std::endl;
        ...
        {
            // sender not found
            std::cout << "sender (lspid=" << flow.Filter_Spec_Object.Lsp_Id << ") not found in RSB, adding..." << std::endl;
...
void RSVP::removeRsbFilter(ResvStateBlock_t *rsb, unsigned int index)
{
    ...
    std::cout << "removing filter (lspid=" << lspid << ")" << std::endl;
...
void RSVP::removeRSB(ResvStateBlock_t *rsb)
{
    ...
    std::cout << "removing empty RSB " << rsb->id << std::endl;
...
void RSVP::removePSB(PathStateBlock_t *psb)
{
    ...
    std::cout << "removing PSB " << psb->id << " (lspid " << lspid << ")" << std::endl;
    ...
    if (rsb)
    {
        std::cout << "reservation state present, will be removed too" << std::endl;
...
bool RSVP::evalNextHopInterface(IPAddress destAddr, const EroVector& ERO, IPAddress& OI)
{
    if (ERO.size() > 0)
    {
        // explicit routing
        ...
        if (ERO[0].L)
        {
            ...
            if (!ie)
            {
                std::cout << "next (loose) hop address " << ERO[0].node << " is currently unroutable" << std::endl;
                return false;
            }
    ...
    else
    {
        ...
        if (!tedmod->isLocalAddress(destAddr))
        {
            ...
            if (!ie)
            {
                std::cout << "destination address " << destAddr << " is currently unroutable" << std::endl;
                return false;
            }
...
RSVP::PathStateBlock_t* RSVP::createPSB(RSVPPathMsg *msg)
{
    ...
    std::cout << "created new PSB " << cPSB->id << std::endl;
...
RSVP::PathStateBlock_t* RSVP::createIngressPSB(const traffic_session_t& session, const traffic_path_t& path)
{
    ...
    std::cout << "CACCheck passed, creating PSB" << std::endl;
...
RSVP::ResvStateBlock_t* RSVP::createEgressRSB(PathStateBlock_t *psb)
{
    ...
    std::cout << "created new (egress) RSB " << rsb->id << std::endl;

    return rsb;
}

void RSVP::handleMessage(inet::cMessage *msg)
{
...
{
    std::cout << "Received RSVP_HELLO" << std::endl;
    //print(msg);
    ...
    std::cout << "hello sender " << peer;
    if (request) std::cout << " REQ";
    if (ack) std::cout << " ACK";
    std::cout << std::endl;
    ...
    if (failure || !h->ok)
    {
        ...
        std::cout << "local peer " << peer << " is now considered up and running" << std::endl;

        recoveryEvent(peer);

        // if peer was considered down, we have stopped sending hellos: resume now
        if (!h->timer->isScheduled())
            scheduleAt(inet::simTime(), h->timer);
    }
    ...
    scheduleAt(inet::simTime() + helloTimeout, h->timeout);
}

void RSVP::processPathErrMsg(RSVPPathError* msg)
{
    std::cout << "Received PATH_ERROR" << std::endl;
    //print(msg);
    ...
    if (!psb)
    {
        std::cout << "matching PSB not found, ignoring error message" << std::endl;
        delete msg;
        return;
    }
    ...
    {
        std::cout << "forwarding error message to PHOP (" << psb->Previous_Hop_Address << ")" << std::endl;
    ...
    {
        std::cout << "error reached ingress router" << std::endl;
...
{
    std::cout << "Received PATH_TEAR" << std::endl;
    //print(msg);
    ...
    if (!psb)
    {
        std::cout << "received PATH_TEAR for nonexisting lspid=" << lspid << std::endl;
        delete msg;
        return;
    }
    ...
    for (PSBVector::iterator it = PSBList.begin(); it != PSBList.end(); it++)
    {
        ...
        if (!msg->getForce())
        {
            std::cout << "merging backup tunnel exists and force flag is not set, ignoring teardown" << std::endl;
            delete msg;
            return;
        }

        std::cout << "merging backup must be removed too" << std::endl;
    ...
    if (psb->ERO.size() > 0)
    {
        std::cout << "forward teardown downstream" << std::endl;
...
{
    std::cout << "Received PATH_MESSAGE" << std::endl;
    print(msg);
...
{
    std::cout << "Received RESV_MESSAGE" << std::endl;
    print(msg);
    ...
    for (unsigned int m = 0; m < msg->getFlowDescriptor().size(); m++)
    {
        ...
        if (!psb)
        {
            std::cout << "matching PSB not found for lspid=" << msg->getFlowDescriptor()[m].Filter_Spec_Object.Lsp_Id << std::endl;
    ...
    {
        std::cout << "no matching PSB found" << std::endl;
        delete msg;
        return;
    }
...
void RSVP::recoveryEvent(IPAddress peer)
{
...
void RSVP::pathProblem(PathStateBlock_t *psb)
{
    ...
    std::cout << "sending PathTear to " << nextHop << std::endl;
    ...
    if (p->permanent)
    {
        std::cout << "this path is permanent, we will try to re-create it later" << std::endl;
    ...
    std::cout << "removing PSB" << std::endl;
...
void RSVP::processPATH_NOTIFY(PathNotifyMsg* msg)
{
    ...
    switch(msg->getStatus())
    {
        ...
        case PATH_CREATED:
            std::cout << "Path successfully established" << std::endl;
            break;
...
void RSVP::addSession(const inet::cXMLElement& node)
{
    Enter_Method_Silent();
...
void RSVP::delSession(const inet::cXMLElement& node)
{
    Enter_Method_Silent();
    ...
    const inet::cXMLElement *paths = getUniqueChildIfExists(&node, "paths");
    inet::cXMLElementList pathList;
    if (paths)
    {
    ...
    for (it = session->paths.begin(); it != session->paths.end(); it++)
    {
        ...
        if (paths)
        {
            ...
            for (inet::cXMLElementList::iterator p=pathList.begin(); p != pathList.end(); p++)
            {
...
void RSVP::processCommand(const inet::cXMLElement& node)
{
...
void RSVP::sendPathTearMessage(IPAddress peerIP, const SessionObj_t& session, const SenderTemplateObj_t& sender, IPAddress LIH, IPAddress NHOP, bool force)
{
...
void RSVP::sendPathErrorMessage(SessionObj_t session, SenderTemplateObj_t sender, SenderTspecObj_t tspec, IPAddress nextHop, int errCode)
{
...
void RSVP::sendToIP(inet::cMessage *msg, IPAddress destAddr)
{
...
void RSVP::scheduleTimeout(PathStateBlock_t *psbEle)
{
    ...
    scheduleAt(inet::simTime() + PSB_TIMEOUT_INTERVAL, psbEle->timeoutMsg);
}

void RSVP::scheduleRefreshTimer(PathStateBlock_t *psbEle, inet::simtime_t delay)
{
    ASSERT(psbEle);
    ...
    std::cout << "scheduling PSB " << psbEle->id << " refresh " << (inet::simTime() + delay) << std::endl;

    scheduleAt(inet::simTime() + delay, psbEle->timerMsg);
}
...
void RSVP::scheduleTimeout(ResvStateBlock_t *rsbEle)
{
    ...
    scheduleAt(inet::simTime() + RSB_TIMEOUT_INTERVAL, rsbEle->timeoutMsg);
}

void RSVP::scheduleRefreshTimer(ResvStateBlock_t *rsbEle, inet::simtime_t delay)
{
    ASSERT(rsbEle);
    ...
    scheduleAt(inet::simTime() + delay, rsbEle->refreshTimerMsg);
}
...
void RSVP::scheduleCommitTimer(ResvStateBlock_t *rsbEle)
{
    ...
    scheduleAt(inet::simTime(), rsbEle->commitTimerMsg);
}
...
{
    std::cout << "PATH_MESSAGE: lspid " << p->getLspId() << " ERO " << vectorToString(p->getERO()) << std::endl;
}

void RSVP::print(RSVPResvMsg *r)
{
    std::cout << "RESV_MESSAGE: " << std::endl;
    for (unsigned int i = 0; i < r->getFlowDescriptor().size(); i++)
    {
        std::cout << " lspid " << r->getFlowDescriptor()[i].Filter_Spec_Object.Lsp_Id <<
            " label " << r->getFlowDescriptor()[i].label << std::endl;
    }
}
