...
{
  public:
    typedef std::vector<omnetpp::cMessage*> MsgPtrVector;  // TODO verify this is really needed --Andras
	...
	bool isHomeAgent;	//is the router also a Home Agent (RFC 3775-MIPv6)...Zarrar Yousaf 09.03.07
        ...
        inet::simtime_t reachabilityExpires; // reachabilityLastConfirmed+reachableTime
        short numProbesSent;
        omnetpp::cMessage *nudTimeoutEvent; // DELAY or PROBE timer
        ...
        int numOfARNSSent;
        omnetpp::cMessage *arTimer;//Address Resolution self-message timer
        ...
        inet::simtime_t routerExpiryTime;   // time when router lifetime expires
    };
  ...
  protected:
    omnetpp::cSimpleModule &neighbourDiscovery; // for cancelAndDelete() calls
    NeighbourMap neighbourMap;

  public:
    IPv6NeighbourCache(omnetpp::cSimpleModule &neighbourDiscovery);
    ...
    virtual Neighbour *addRouter(const IPv6Address& addr, int interfaceID,
            inet::simtime_t expiryTime, bool isHomeAgent = false); // added HA flag, 3.9.07 - CB

    /** Creates and initializes a router entry (isRouter=isDefaultRouter=true), MAC address and state=STALE. */
    virtual Neighbour *addRouter(const IPv6Address& addr, int interfaceID,
                         MACAddress macAddress, inet::simtime_t expiryTime, bool isHomeAgent = false); // added HA flag, 3.9.07 - CB
