...
//#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\ospfv2\router\OSPFcommon.h"
//#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\ospfv2\router\OSPFArea.h"
//#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\ospfv2\interface\OSPFInterface.h"
...
// */
//void OSPFRouting::handleMessage(inet::cMessage *msg)
//{
...
//    if (!ie)
//        throw inet::cException("error reading XML config: IInterfaceTable contains no interface named '%s'", name.c_str());
...
// */
//void OSPFRouting::GetAreaListFromXML(const inet::cXMLElement& routerNode, std::map<std::string, int>& areaList) const
//{
//    inet::cXMLElementList routerConfig = routerNode.getChildren();
//    for (inet::cXMLElementList::iterator routerConfigIt = routerConfig.begin(); routerConfigIt != routerConfig.end(); routerConfigIt++) {
...
// */
//void OSPFRouting::LoadAreaFromXML(const inet::cXMLElement& asConfig, const std::string& areaID)
//{
//    std::string areaXPath("Area[@id='");
//    areaXPath += areaID;
//    areaXPath += "']";
//
//    inet::cXMLElement* areaConfig = asConfig.getElementByPath(areaXPath.c_str());
//    if (areaConfig == NULL) {
//        error("No configuration for Area ID: %s", areaID.c_str());
//    }
//    else {
//        std::cout << "    loading info for Area id = " << areaID << "\n";
//    }
//
//    OSPF::Area* area = new OSPF::Area(ULongFromAddressString(areaID.c_str()));
//    inet::cXMLElementList areaDetails = areaConfig->getChildren();
//    for (inet::cXMLElementList::iterator arIt = areaDetails.begin(); arIt != areaDetails.end(); arIt++) {
...
// */
//void OSPFRouting::LoadInterfaceParameters(const inet::cXMLElement& ifConfig)
//{
...
//
//    std::cout << "        loading " << interfaceType << " " << ifName << " ifIndex[" << ifIndex << "]\n";
//
...
//    inet::cXMLElementList ifDetails = ifConfig.getChildren();
//
//    for (inet::cXMLElementList::iterator ifElemIt = ifDetails.begin(); ifElemIt != ifDetails.end(); ifElemIt++) {
...
//            inet::cXMLElementList neighborList = (*ifElemIt)->getChildren();
//            for (inet::cXMLElementList::iterator neighborIt = neighborList.begin(); neighborIt != neighborList.end(); neighborIt++) {
...
// */
//void OSPFRouting::LoadExternalRoute(const inet::cXMLElement& externalRouteConfig)
//{
...
//
//    std::cout << "        loading ExternalInterface " << ifName << " ifIndex[" << ifIndex << "]\n";
//
//    inet::cXMLElementList ifDetails = externalRouteConfig.getChildren();
//    for (inet::cXMLElementList::iterator exElemIt = ifDetails.begin(); exElemIt != ifDetails.end(); exElemIt++) {
...
// */
//void OSPFRouting::LoadHostRoute(const inet::cXMLElement& hostRouteConfig)
//{
...
//
//    std::cout << "        loading HostInterface " << ifName << " ifIndex[" << static_cast<short> (hostParameters.ifIndex) << "]\n";
//
//    inet::cXMLElementList ifDetails = hostRouteConfig.getChildren();
//    for (inet::cXMLElementList::iterator hostElemIt = ifDetails.begin(); hostElemIt != ifDetails.end(); hostElemIt++) {
...
// */
//void OSPFRouting::LoadVirtualLink(const inet::cXMLElement& virtualLinkConfig)
//{
//    OSPF::Interface* intf     = new OSPF::Interface;
//    std::string      endPoint = virtualLinkConfig.getAttribute("endPointRouterID");
//    OSPF::Neighbor*  neighbor = new OSPF::Neighbor;
//
//    std::cout << "        loading VirtualLink to " << endPoint << "\n";
//
//    intf->SetType(OSPF::Interface::Virtual);
//    neighbor->SetNeighborID(ULongFromAddressString(endPoint.c_str()));
//    intf->AddNeighbor(neighbor);
//
//    inet::cXMLElementList ifDetails = virtualLinkConfig.getChildren();
//    for (inet::cXMLElementList::iterator ifElemIt = ifDetails.begin(); ifElemIt != ifDetails.end(); ifElemIt++) {
//        std::string nodeName = (*ifElemIt)->getTagName();
...
//{
//    inet::cXMLElement *asConfig = nullptr;
////
////        // Memuat file XML menggunakan parseFile
////        asConfig = new cXMLElement();
////        if (asConfig->parseFile(filename)) {
////            // Jika berhasil memuat, lanjutkan dengan pengolahan data XML
////            EV << "XML File Loaded Successfully\n";
////            // ... Proses XML sesuai kebutuhan Anda
////        } else {
////            // Tangani error jika file tidak dapat dimuat
////            throw cRuntimeError("Failed to load XML document: %s", filename);
////        }
//
...
//    routerXPath += "']";
//
//    inet::cXMLElement* routerNode = asConfig->getElementByPath(routerXPath.c_str());
//    if (routerNode == NULL) {
//        error("No configuration for Router ID: %s", routerId.str().c_str());
//    }
//    else {
//        std::cout << "OSPFRouting: Loading info for Router id = " << routerId.str() << "\n";
//    }
//
