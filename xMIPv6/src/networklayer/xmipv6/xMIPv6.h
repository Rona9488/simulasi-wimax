...
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\xmipv6\MobilityHeader.h"
...
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\xmipv6\BindingUpdateListAccess.h"
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\xmipv6\BindingCache.h"
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\xmipv6\BindingCacheAccess.h"
// Added by CB
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\ipv6\IPv6TunnelingAccess.h"
// 14.01.08 - CB
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\base\NotificationBoard.h"
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\contract\IPAddressResolver.h" // Adam
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\wimax\wmaxctrl.h"//============= Adam 14-09-2011 =====================s
 ...
 */
class INET_API xMIPv6 : public inet::cSimpleModule
{
  public:
	  virtual ~xMIPv6();
	  inet::simtime_t FMIPv6StartTime;//============= Adam 14-09-2011 =====================
  protected:
	IInterfaceTable* ift;
//============= Adam 14-09-2011 =====================
    InterfaceTable* ift2;
    WMaxCtrlSS* wmaxctrl;
    inet::cOutVector FMIPv6PrepDuration;
    inet::cOutVector OpoznienieRouteOptim;
    inet::cOutVector OpoznienieBUtoHA;
//============= Adam, end  16-09-2011==================
	RoutingTable6* rt6;
	...
	inet::cOutVector statVectorBUtoHA, statVectorBUtoCN, statVectorBUtoMN;
	// 1 means BA from HA, 2 means BA from CN
	inet::cOutVector statVectorBAtoMN;
	// 1 means BA to register BU, 2 means BA to deregister BU message
	// 3 means invalid BA
	inet::cOutVector statVectorBAfromHA, statVectorBAfromCN;

	inet::cOutVector statVectorHoTItoCN, statVectorCoTItoCN;
	inet::cOutVector statVectorHoTtoMN, statVectorCoTtoMN;
	inet::cOutVector statVectorHoTfromCN, statVectorCoTfromCN;
	...
	{
	public:
	    inet::cMessage* timer; // pointer to the scheduled timer message
		...
		inet::simtime_t ackTimeout; // timeout for the Ack
		inet::simtime_t nextScheduledTime; // time when the corrsponding message is supposed to be sent
	...
	struct Key
	{
		...
		Key(IPv6Address _dest, int _interfaceID, int _type) {dest=_dest;interfaceID=_interfaceID;type=_type;}
	...
	public:
		...
		inet::simtime_t presentSentTimeBU;//stores the present time at which BU is/was sent
  ...
  protected:
	 ...
	 */
	void createBUTimer(const IPv6Address& buDest, InterfaceEntry* ie, const uint lifeTime, bool homeRegistration); // 4.9.07 - CB
	 ...
	 */
	void sendPeriodicBU(inet::cMessage *msg); //06.06.07
	 ...
	 */
	void updateBUL(BindingUpdate* bu, const IPv6Address& dest, const IPv6Address& CoA, InterfaceEntry* ie, const inet::simtime_t sendTime); //04.06.07
	 ...
	 */
	void sendMobilityMessageToIPv6Module(inet::cMessage *msg, const IPv6Address& destAddr, const IPv6Address& srcAddr = IPv6Address::UNSPECIFIED_ADDRESS, int interfaceId = -1, inet::simtime_t sendTime = 0); // overloaded for use at CN - CB
	...
	void createAndSendBAMessage(const IPv6Address& src, const IPv6Address& dest, IPv6ControlInfo* ctrlInfo, const BAStatus& baStatus, const uint baSeq,
			const int bindingAuthorizationData, const uint lifeTime, inet::simtime_t sendTime = 0); // 14.9.07 - CB
...
//
  ...
  protected:
	 ...
	 */
	void createTestInitTimer(MobilityHeader* testInit, const IPv6Address& dest, InterfaceEntry* ie, inet::simtime_t sendTime = 0); // 27.08.07 - CB
	 ...
	 */
	void sendTestInit(inet::cMessage *msg); // 27.08.07 - CB
	 ...
	 */
	void resetBUIfEntry(const IPv6Address& dest, int interfaceID, inet::simtime_t retransmissionTime);
...
//
	 ...
	 */
	void sendPeriodicBRR(inet::cMessage *msg); // 18.09.07 - CB
...
//
	 ...
	 */
	void createBULEntryExpiryTimer(BindingUpdateList::BindingUpdateListEntry* entry, InterfaceEntry* ie, inet::simtime_t scheduledTime);

	/**
	 * Handles the situation of a BUL expiry. Either a BU is sent in advance for renewal or the BUL entry is removed.
	 */
	void handleBULExpiry(inet::cMessage* msg);

	/**
	 * Creates or overwrites a timer for BC expiry that fires at provided scheduledTime.
	 */
	void createBCEntryExpiryTimer(IPv6Address& HoA, InterfaceEntry* ie, inet::simtime_t scheduledTime);

	/**
	 * Handles the expiry of a BC entry.
	 * Entry is removed from BC and tunnels/routing paths are destroyed.
	 */
	void handleBCExpiry(inet::cMessage* msg);
...
//
	 ...
	 */
	void createHomeTokenEntryExpiryTimer(IPv6Address& cnAddr, InterfaceEntry* ie, inet::simtime_t scheduledTime) { createTokenEntryExpiryTimer(cnAddr, ie, scheduledTime, KEY_HTOKEN_EXP); };

	/**
	 * Creates or overwrites a timer for care-of keygen token expiry that fires at provided scheduledTime.
	 */
	void createCareOfTokenEntryExpiryTimer(IPv6Address& cnAddr, InterfaceEntry* ie, inet::simtime_t scheduledTime) { createTokenEntryExpiryTimer(cnAddr, ie, scheduledTime, KEY_CTOKEN_EXP); };
...
private:
	 ...
	 */
	void createTokenEntryExpiryTimer(IPv6Address& cnAddr, InterfaceEntry* ie, inet::simtime_t scheduledTime, int tokenType);

	/**
	 * Handles the event that indicates that a {care-of,home} keygen token has expired.
	 */
	void handleTokenExpiry(inet::cMessage* msg);
