...
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\wimax\wmaxctrlaccess.h"//============= Adam 14-09-2011 =====================s
...
void xMIPv6::initialize(int stage)
{
	...
	if (stage == 0)
	{
    	std::cout << "Initializing xMIPv6 module" << std::endl;
        ...
        OpoznienieBUtoHA.setName("Czas pomi�dzy konfiguracja L3 a wyslaniem BU do HA");
...
void xMIPv6::handleMessage(inet::cMessage *msg)
{
    if ( msg->isSelfMessage() )
    {
        ...
        {
            std::cout << "Periodic BU Timeout Message Received\n";
            sendPeriodicBU(msg);
        }
        else if (msg->getKind()==MK_SEND_PERIODIC_BR)
        {
            std::cout << "Periodic BRR Timeout Message Received\n";
            sendPeriodicBRR(msg);
        }
        else if (msg->getKind()==MK_SEND_TEST_INIT) // 28.08.07 - CB
        {
        	std::cout << "HoTI/CoTI Timeout Message Received\n";
        	sendTestInit(msg);
        }
        else if (msg->getKind()==MK_BUL_EXPIRY) // 12.06.08 - CB
        {
        	std::cout << "BUL Expiry Timeout Message Received\n";
        	handleBULExpiry(msg);
        }
        ...
        {
        	std::cout << "RR token expired" << std::endl;
    ...
    {
    	std::cout <<" Received MIPv6 related message" << std::endl;
    ...
    else if ( dynamic_cast<IPv6Datagram*>(msg) )
    {
    	...
    	if ( dynamic_cast<IPv6RoutingHeader*>(eh) )
    		...
    		throw inet::cException("Unknown Extension Header.");
    }
    else
        throw inet::cException("Unknown message type received.");
}


void xMIPv6::processMobilityMessage(MobilityHeader* mipv6Msg, IPv6ControlInfo* ctrlInfo)
{
	std::cout <<"Processing of MIPv6 related mobility message" << std::endl;
	if ( dynamic_cast<BindingUpdate*>(mipv6Msg) )
	{
		std::cout <<"Message recognised as BINDING UPDATE (BU)" << std::endl;
	...
	{
		std::cout <<"Message recognised as BINDING ACKNOWLEDGEMENT (BA)" << std::endl;
...
//====== Adam 08-09-2011 =============================================
	...
	{
        std::cout <<"Message recognised as Router Solicitation For Proxy Advertisement" << std::endl;
...
void xMIPv6::CreateAndSendRtSolPr( int BSID ) 
{
    ...
    std::cout << "Sending RtSolPr to destAddr = " << destAddrIP << "  srcAddr = " << srcAddrIP.str() << std::endl;
...
void xMIPv6::CreateAndSendPrRtAdv( RouterSolicitationForProxyAdvertisement* mipv6Msg, IPv6ControlInfo* ctrlInfo )
{
    ...
    std::cout << "Sending PrRtAdv to destAddr = " << destAddr.str() << "  srcAddr = " << srcAddr<< std::endl;
...
{
    std::cout << "xMIPv6::processPrRtAdvMessage" << std::endl;
    std::cout << "mipv6Msg->getAP_ID() = " << mipv6Msg->getAP_ID() << std::endl;
    std::cout << "mipv6Msg->getL2_Addr() = " << mipv6Msg->getL2_Addr().str() << std::endl;
    std::cout << "mipv6Msg->getL3_Addr() = " << mipv6Msg->getL3_Addr().str() << std::endl;
    std::cout << "mipv6Msg->getPrefix() = " << mipv6Msg->getPrefix().str() << std::endl;
    std::cout << "mipv6Msg->getPrefixLength() = " << mipv6Msg->getPrefixLength() << std::endl;
    ...
    std::cout << "Handover NCoA = " << ift2->getHandover_NCoA() << std::endl;
    ...
    IPv6Address destAddr = ctrlInfo->getSrcAddr(); // odsylam spowrotem
    std::cout << "Sending FBU to destAddr = " << destAddr.str() << "  srcAddr = " << srcAddr<< std::endl;
...
{   // przekazuje FBU do NAR pod postacia HI
    std::cout << "xMIPv6::processFBUMessage" << std::endl;
    ...
    std::cout << "Sending HI to destAddr = " << destAddr.str() << "  srcAddr = " << srcAddr.str()<< std::endl;
...
void xMIPv6::processHAckMessage( HandoverAcknowledge* mipv6Msg, IPv6ControlInfo* ctrlInfo )
{
    ...
    std::cout << "Sending HI to destAddr = " << destAddr2.str() << "  srcAddr = " << srcAddr2.str()<< std::endl;
...
void xMIPv6::processFBAMessage( FastBindingAcknowledge* mipv6Msg, IPv6ControlInfo* ctrlInfo )
{
    if( mipv6Msg->getHandover_accepted() == 1 ){    // jesli handover przyjety przez NAR
        ...
        if( this->getParentModule()->getParentModule()->getSubmodule("ssInfo") ){    // obluga MN
            std::cout << "obluga MN" << std::endl;
            std::cout << "FMIPv6PrepDuration" << std::endl << "inet::simTime() = " << inet::simTime() << std::endl << "FMIPv6StartTime = " << FMIPv6StartTime << std::endl;
            FMIPv6PrepDuration.record(inet::simTime() - FMIPv6StartTime );
            // ss->xmipv6->CreateAndSendRtSolPr( nearestBS );
            // inet::cMessage *msg = check_and_cast<inet::cMessage*>(mipv6Msg) );
        ...
        if( this->getParentModule()->getParentModule()->getSubmodule("bsIPv6") ){    // obluga NAR
            std::cout << "BS[0] ROZPOCZAL TUNELOWANIE PCOA->NCOA" << std::endl;     // POPRAWIC
...
void xMIPv6::initiateMIPv6Protocol(InterfaceEntry *ie, IPv6Address& CoA)
{
	...
	if( !( ie->isLoopback() ) && rt6->isMobileNode() )
 	{
		std::cout << "Initiating Mobile IPv6 protocol..." << std::endl;
        ...
        std::cout << "createBUTimer" << std::endl;
...
void xMIPv6::returningHome(const IPv6Address& CoA, InterfaceEntry* ie)
{
	...
	std::cout << "MIPv6 Returning home procedure..." << std::endl;
...
void xMIPv6::createBUTimer(const IPv6Address& buDest, InterfaceEntry* ie, const uint lifeTime, bool homeRegistration)
{
	Enter_Method("createBUTimer()");
	std::cout<<"Creating BU timer at sim time: " << inet::simTime() <<" seconds." << std::endl;
	inet::cMessage *buTriggerMsg = new inet::cMessage("sendPeriodicBU", MK_SEND_PERIODIC_BU);
	...
	scheduleAt(inet::simTime(), buTriggerMsg); //Scheduling a message which will trigger a BU towards buIfEntry->dest
}


void xMIPv6::sendPeriodicBU(inet::cMessage *msg)
{
	std::cout <<"Sending periodic BU message at time: " << inet::simTime() << " seconds." << std::endl;
	BUTransmitIfEntry *buIfEntry = (BUTransmitIfEntry*) msg->getContextPointer(); //detaching the corresponding buIfEntry pointer
	//std::cout << "### lifetime of buIfEntry=" << buIfEntry->lifeTime << " and seq#= " << buIfEntry->buSequenceNumber << std::endl;
	InterfaceEntry* ie = buIfEntry->ifEntry; //copy the ie info
	IPv6Address& buDest = buIfEntry->dest;
	buIfEntry->presentSentTimeBU = inet::simTime(); //records the present time at which BU is sent
	...
	else
	{
		...
		if( this->getParentModule()->getParentModule()->getIndex() == 0 ){
			...
			OpoznienieBUtoHA.record( inet::simTime() - wmaxctrl->L3Ready );
		}
	...
	{
		std::cout<<"Crossed maximum BINDACK timeout...resetting to predefined maximum." << std::endl;//buIfEntry->nextBindAckTimeout<<" ++++++\n";
		//ev<<"\n++++Present Sent Time: "<<buIfEntry->presentSentTimeBU<<" Present TimeOut: "<<buIfEntry->ackTimeout<<std::endl;
		...
		//ev<<"\n++++Next Sent Time: "<<buIfEntry->nextScheduledTime<<std::endl;//" Next TimeOut: "<<buIfEntry->nextBindAckTimeout<<std::endl;
		//scheduleAt(buIfEntry->nextScheduledTime, msg);
	}
	std::cout << "Present Sent Time: " << buIfEntry->presentSentTimeBU <<", Present TimeOut: " << buIfEntry->ackTimeout << std::endl;
	std::cout << "Next Sent Time: " << buIfEntry->nextScheduledTime << std::endl;//<<" Next TimeOut: "<<buIfEntry->nextBindAckTimeout<<std::endl;
	scheduleAt(buIfEntry->nextScheduledTime, msg);
}
...
{
	std::cout <<"Creating and sending Binding Update" << std::endl;
	...
	updateBUL( bu, dest, CoA, ie, inet::simTime() );
...
void xMIPv6::updateBUL(BindingUpdate* bu, const IPv6Address& dest, const IPv6Address& CoA, InterfaceEntry* ie, const inet::simtime_t sendTime)
{
	...
	if ( buIfEntry == NULL  )
	{
		std::cout << "No scheduled BU entry available!\n";
		return;
	}

	//inet::simtime_t sentTime = buIfEntry->presentSentTimeBU;
	//inet::simtime_t nextSentTime = buIfEntry->nextScheduledTime;

	//ASSERT(bul);
	bul->addOrUpdateBUL(dest, HoA, CoA, buLife, buSeq, sendTime);//, nextSentTime); //updates the binding Update List
	//std::cout << "#### Updated BUL with lifetime=" << buLife << "and sentTime=" << sentTime << std::endl;
}
...
void xMIPv6::sendMobilityMessageToIPv6Module(inet::cMessage *msg, const IPv6Address& destAddr,
    const IPv6Address& srcAddr, int interfaceId, inet::simtime_t sendTime) // overloaded for use at CN - CB
{

	std::cout <<"Appending ControlInfo to mobility message\n";
    ...
    std::cout <<"ControlInfo appended successfully. Sending mobility message to IPv6 module\n";

    std::cout << "controlInfo: DestAddr=" << controlInfo->getDestAddr()
       << "SrcAddr=" << controlInfo->getSrcAddr()
       << "InterfaceId=" << controlInfo->getInterfaceId() << std::endl;
...
std::cout <<"\n<<======THIS IS THE (SMALL) ROUTINE FOR APPENDING CONTROL INFO TO MOBILITY MESSAGES =====>>\n";
	...
	std::cout << "controlInfo: DestAddr=" << controlInfo->destAddr()
	   << "SrcAddr=" << controlInfo->srcAddr()
	   << "InterfaceId=" << controlInfo->interfaceId() << std::endl;
	...
	if( dynamic_cast<BindingAcknowledgement*>(msg) && rt6->isHomeAgent() )
	{
		...
		{
			std::cout << "Message is positive BA with status: "<< ba->getStatus() << ", sending it with a PSEUDO DAD Delay of 60 sec"<<std::endl;
...
{
	std::cout << "Entered BU processing method" << std::endl;
	...
	*/
	...
	{
		std::cout << "Wrong Node: not HA or CN" << std::endl;
	...
	if (validBUMessage)
	{
		...
		if ( (buLifetime == 0) || (CoA == HoA) )
		{
			...
			if ( rt6->isHomeAgent() && ! validateBUderegisterMessage(bu, ctrlInfo) ) // HAs have to validate the BU
			{
				...
				std::cout << "Error: Not HA for this MN. Responding with appropirate BA...\n";
				delete bu;
				delete ctrlInfo;
				return;
			}
		...
		else
		{
			...
			createBCEntryExpiryTimer(HoA, ift->getInterfaceById( ctrlInfo->getInterfaceId() ), inet::simTime()+buLifetime);
			...
			{
				std::cout<<"BU Validated as OK: ACK FLAG NOT SET"<< std::endl;
	...
	{
		std::cout << "BU Validation Failed: Dropping message"<<std::endl;
...
bool xMIPv6::validateBUMessage(BindingUpdate *bu, IPv6ControlInfo *ctrlInfo)
{
	...
	{
		std::cout<<"BU Validation Failed: SrcAdress is not unicast Global !"<<std::endl;
		std::cout<<"Dropping unvalidated BU message"<<std::endl;
		bubble ("!! BU Validation Failed !!");
		return false;//result = false;
	}
	if ( ! (homeAddress.isGlobal() && homeAddress.isUnicast() ) )
	{
		std::cout<<"BU Validation Failed: Home Adress of MN is not unicast Global !"<<std::endl;
		bubble ("!! BU Validation Failed !!");
		std::cout<<"Dropping unvalidated BU message"<<std::endl;
		return false;//result = false;
	}
	...
	{
		std::cout<<"BU Validation Failed: Received Seq#: " << seqNumber << " is LESS THAN in BC: " << bcSeqNumber <<std::endl;
		bubble ("!! BU Validation Failed !!");
		std::cout<<"Dropping unvalidated BU message"<<std::endl;
	...
	if (! bu->getHomeRegistrationFlag() )
	{
		if ( bc->getHomeRegistration(homeAddress) != bu->getHomeRegistrationFlag() ) // update 10.9.07 - CB
		{
			...
			std::cout << "BU Validation Failed: home registration flag in binding cache has different value than in the binding update" << std::endl;
		...
		{
			std::cout << "homeToken=" << homeToken << ", careOfToken=" << careOfToken
				<< " , bindAuthData=" << bindAuthData << ", BU data=" << bu->getBindingAuthorizationData() << std::endl;

			if ( (bu->getBindingAuthorizationData() != bindAuthData) && (bu->getBindingAuthorizationData() != homeToken) ) // this is quick and dirty -> TODO
			{
				std::cout << "BU Validation Failed: Binding Authorization Data invalid!" << std::endl;
				return false;
			}
		}
	...
	std::cout<<"BU validation passed"<<std::endl;
	if (!inet::getEnvir()->isGUI())
		bubble ("BU Validated");
...
void xMIPv6::createAndSendBAMessage(const IPv6Address& src, const IPv6Address& dest, IPv6ControlInfo* ctrlInfo, const BAStatus& baStatus,
		const uint baSeq, const int bindingAuthorizationData, const uint lifeTime, const inet::simtime_t sendTime)
{
	std::cout << "Entered createAndSendBAMessage() method" << std::endl;
...
void xMIPv6::processBAMessage(BindingAcknowledgement* ba, IPv6ControlInfo* ctrlInfo)
{
	...
	if ( rt6->isMobileNode() )
	{
		...
		{
			std::cout << "Discarding invalid BAck...\n";
			delete ctrlInfo;
			delete ba;
		...
		{
			// update 21.9.07 - CB
			std::cout << "Binding was accepted." << std::endl;
			...
			else
			{
				...
				int l_remain = entry->bindingLifetime - ( SIMTIME_DBL(inet::simTime() - entry->sentTime) );
				int x = l_remain - (l_update - l_ack);
				entry->bindingLifetime = x > 0 ? x : 0;
				entry->bindingExpiry = inet::simTime() + entry->bindingLifetime;
				...
				inet::simtime_t scheduledTime = entry->bindingExpiry - PRE_BINDING_EXPIRY;
				scheduledTime = scheduledTime > 0 ? scheduledTime : 0;

				/*std::cout << "l_ack=" << l_ack << ", l_update=" << l_update << ", l_remain=" << l_remain << ", x=" << x << std::endl;
				std::cout << "entry->bindingLifetime=" << entry->bindingLifetime << " and entry->bindingExpiry=" << entry->bindingExpiry << std::endl;*/
				std::cout << "Scheduling BULEntryExpiryTimer for " << scheduledTime << std::endl;
...
bool xMIPv6::validateBAck(const BindingAcknowledgement& ba, const IPv6ControlInfo* ctrlInfo)
{
	...
	if ( cnAddress != ie->ipv6Data()->getHomeAgentAddress() ) // BAck comes from a CN and not from the HA
	{
		...
		{
			std::cout << "BA Validation Failed: Binding Authorization Data invalid !"<<std::endl;
			return false;
		}
	}
	...
	{
		std::cout << "BA Validation Failed: Sequence number from BA does not match the one from the BUL!!"<<std::endl;
		return false;
	}
...
void xMIPv6::triggerRouteOptimization(const IPv6Address& destAddress, const IPv6Address& HoA, InterfaceEntry* ie)
{
	...
	{
		std::cout << "Route Optimization for: " << destAddress <<" already triggered";
		...
		if ( (bul->getCoA(destAddress) != ie->ipv6Data()->getGlobalAddress(IPv6InterfaceData::CoA)) || bul->getMobilityState(destAddress) == BindingUpdateList::NONE )
		{
			// we have a new CoA compared to BUL entry -> redo RO
			initReturnRoutability(destAddress, ie);
			std::cout << " ...moved to new network. Initializing RO.";
		}
	...
	{
		if ( (!bul->isInBindingUpdateList(destAddress, HoA)) || (bul->getMobilityState(destAddress) == BindingUpdateList::RR) )
		{
			std::cout <<"Initialise Route Optimization for: " << destAddress <<"\n";
			initReturnRoutability(destAddress, ie);
...
void xMIPv6::initReturnRoutability(const IPv6Address& cnDest, InterfaceEntry* ie)
{
	...
	std::cout <<"Initiating Return Routability..." << std::endl;
	...
	//{
		...
		//if ( (cnEntry->tokenH != UNDEFINED_TOKEN) && ( cnEntry->sentHoTI + ie->ipv6()->_maxTokenLifeTime() > inet::simTime() ) )
		if ( bul->isHomeTokenAvailable(cnDest, ie) )
		{
			std::cout << "Valid home token available - sending HoTI later." << std::endl;
			sendHoTI = false;
		}
		//else
		//	delete HoTI;

		/*if ( (cnEntry->tokenC != UNDEFINED_TOKEN) && ( cnEntry->sentCoTI + ie->ipv6()->_maxTokenLifeTime() > inet::simTime() ) )*/
		if ( bul->isCareOfTokenAvailable(cnDest, ie) )
		{
			std::cout << "Valid care-of token available - sending CoTI later." << std::endl;
		...
		//else
		//	delete CoTI;
        std::cout << "sendHoTI = " << sendHoTI << "   sendCoTI = " << sendCoTI << std::endl;
    ...
    std::cout << "recentlySentHOTI = " << bul->recentlySentHOTI(cnDest, ie) << std::endl;
	if ( sendHoTI && !bul->recentlySentHOTI(cnDest, ie) )
	{
		if( this->getParentModule()->getParentModule()->getIndex() == 0 ){
			wmaxctrl = WMaxCtrlSSAccess().get();
			std::cout << "OpoznienieRouteOptim" << std::endl << "wmaxctrl->L3Ready = " << wmaxctrl->L3Ready << std::endl << "inet::simTime() = " << inet::simTime() << std::endl;
			OpoznienieRouteOptim.record( inet::simTime() - wmaxctrl->L3Ready );
		}
    ...
    std::cout << "recentlySentCOTI = " << bul->recentlySentCOTI(cnDest, ie) << std::endl;
...
void xMIPv6::createTestInitTimer(MobilityHeader* testInit, const IPv6Address& dest, InterfaceEntry* ie, inet::simtime_t sendTime)
{
	std::cout<<"\n++++++++++TEST INIT TIMER CREATED AT SIM TIME: "<<inet::simTime()<<" seconds+++++++++++++++++ \n";
	inet::cMessage *testInitTriggerMsg = new inet::cMessage("sendTestInit", MK_SEND_TEST_INIT);
	...
	tiIfEntry->nextScheduledTime = inet::simTime(); // we send the HoTI/CoTI now
	...
	scheduleAt(inet::simTime()+sendTime, testInitTriggerMsg);
}


void xMIPv6::sendTestInit(inet::cMessage* msg)
{
	...
	if ( dynamic_cast<HomeTestInit*>(tiIfEntry->testInitMsg) )
	{
		...
		// update cache
		bul->addOrUpdateBUL(tiIfEntry->dest, HoA, inet::simTime(), HoTI->getHomeInitCookie(), true);
	...
	else // must be of type CareOfTestInit*
	{
		...
		// update cache
		bul->addOrUpdateBUL(tiIfEntry->dest, CoA, inet::simTime(), CoTI->getCareOfInitCookie(), false);
	...
	tiIfEntry->nextScheduledTime = tiIfEntry->ackTimeout + inet::simTime();
	...
	std::cout << "Scheduled next HoTI/CoTI for time=" << tiIfEntry->nextScheduledTime
	   << " with timeout=" << tiIfEntry->ackTimeout << " for dest="
	   << tiIfEntry->dest << std::endl;
...
/*void xMIPv6::resetTestInitIfEntry(const IPv6Address& dest, int interfaceID, int msgType)
{
	...
	{
		//std::cout << "+++ No corresponding TimerIfEntry found! +++\n";
		return;
	}
	...
	entry->nextScheduledTime = inet::simTime() + MAX_TOKEN_LIFETIME * TEST_INIT_RETRANS_FACTOR;
	scheduleAt(entry->nextScheduledTime, entry->timer);


	std::cout << "Updated TestTransmitIfEntry and corresponding timer.\n";
...
void xMIPv6::resetBUIfEntry(const IPv6Address& dest, int interfaceID, inet::simtime_t retransmissionTime)
{
	...
	std::cout << "Updated BUTransmitIfEntry and corresponding timer.\n";
}
...
void xMIPv6::processHoTMessage(HomeTest* HoT, IPv6ControlInfo* ctrlInfo)
{
	if (! validateHoTMessage(*HoT, ctrlInfo) )
		std::cout << "HoT validation not passed: dropping message" << std::endl;
	else
	{
		std::cout << "HoT validation passed: updating BUL" << std::endl;
		...
		createHomeTokenEntryExpiryTimer( srcAddr, ift->getInterfaceById(interfaceID), inet::simTime() + ie->ipv6Data()->_maxTokenLifeTime() );
...
bool xMIPv6::validateHoTMessage(const HomeTest& HoT, const IPv6ControlInfo* ctrlInfo)
{
	...
	if ( bulEntry == NULL )
	{
		std::cout << "Invalid HoT: No entry in BUL" << std::endl;
	...
	{
		std::cout << "Invalid HoT: Home keygen token already exists." << std::endl;
	...
	{
		std::cout << "Invalid HoT: Destination Address not HoA." << std::endl;
	...
	{
		std::cout << "Invalid HoT: Cookie value different from the stored one." << std::endl;
		return false;
	}
...
void xMIPv6::processCoTMessage(CareOfTest* CoT, IPv6ControlInfo* ctrlInfo)
{
	if (! validateCoTMessage(*CoT, ctrlInfo) )
		std::cout << "CoT validation not passed: dropping message" << std::endl;
	else
	{
		std::cout << "CoT validation passed: updating BUL" << std::endl;
		...
		// 10.07.08 - CB
		// set token expiry appropriately
		createCareOfTokenEntryExpiryTimer(srcAddr, ift->getInterfaceById(interfaceID), inet::simTime() + ie->ipv6Data()->_maxTokenLifeTime());
...
bool xMIPv6::validateCoTMessage(const CareOfTest& CoT, const IPv6ControlInfo* ctrlInfo)
{
	...
	if ( bulEntry == NULL )
	{
		std::cout << "Invalid CoT: No entry in BUL" << std::endl;
		return false; // no entry in BUL
	}
	if ( bulEntry->sentHoTI == 0 && bulEntry->sentCoTI == 0 )
	{
		std::cout << "Invalid CoT: No RR procedure initialized for this CN." << std::endl;
	...
	{
		std::cout << "Invalid CoT: Already received a care-of keygen token." << std::endl;
		return false; // 0 is expected to indicate "undefined"
	}*/
	...
	{
		std::cout << "Invalid CoT: CoA not valid anymore." << std::endl;
		return false;
	}
	...
	{
		std::cout << "Invalid CoT: Cookie value different from the stored one." << std::endl;
		return false;
	}
...
{
	std::cout << "Checking whether a new BU has to be sent to CN." << std::endl;
	...
	else // case REGISTER, REGISTERED and anything else
	{
		...
		if ( (bul->isBindingAboutToExpire(bulEntry.destAddress) || !bul->isValidBinding(bulEntry.destAddress))
...
void xMIPv6::sendBUtoCN(BindingUpdateList::BindingUpdateListEntry& bulEntry, InterfaceEntry *ie)
{
	...
	std::cout << "Sending BU to CN" << std::endl;
...
{
	//std::cout << "Processing RH2..." << std::endl;
	...
	{
		std::cout << "Invalid RH2 - segments left field must be 1. Dropping packet." << std::endl;
		delete datagram;
		return;
	}
	...
	else
	{
		...
		std::cout << "Invalid RH2 - not a HoA. Dropping packet." << std::endl;
	}
	...
	if ( validRH2 )
	{
		std::cout << "Valid RH2 - copied address from RH2 to datagram" << std::endl;
...
bool xMIPv6::validateType2RH(const IPv6Datagram& datagram, const IPv6RoutingHeader& rh)
{
	...
	{
		std::cout << "Invalid RH2 header: no HoA provided!" << std::endl;
		return false;
	}
	...
	if ( ! HoA.isUnicast() )
	{
		std::cout << "Invalid RH2 header: address in RH2 is not a unicast routable address!" << std::endl;
		return false;
	}
	...
	{
		std::cout << "Invalid RH2 header: scope of Address in RH2 is smaller than the source address of the datagram!" << std::endl;
		return false;
	}
...
void xMIPv6::processHoAOpt(IPv6Datagram* datagram, HomeAddressOption* hoaOpt)
{
	...
	{
		std::cout << "Invalid RH2 destination - no entry in binding cache. Dropping packet." << std::endl;
	...
	if ( validHoAOpt )
	{
		std::cout << "Valid HoA Option - copied address from HoA Option to datagram" << std::endl;
		send(datagram, "toIPv6");
	}
	else
		delete datagram;
...
bool xMIPv6::cancelTimerIfEntry(const IPv6Address& dest, int interfaceID, int msgType)
{
	...
	std::cout << "Deleted TimerIfEntry and corresponding timer.\n";
...
xMIPv6::TimerIfEntry* xMIPv6::getTimerIfEntry(Key& key, int timerType)
{
	...
	if ( pos != transmitIfList.end() )
	{
		...
		else if ( dynamic_cast<TokenExpiryIfEntry*>(pos->second) )
		{
			...
			throw inet::cException("Expected a subclass of TimerIfEntry!");
	...
	else
	{
		...
		switch (timerType)
		{
			...
			case EXPIRY_TYPE_TOKEN:
				...
				break;
			default:
				throw inet::cException("Expected a valid TimerIfEntry type!");
				break;
		}
...
void xMIPv6::removeCoAEntries()
{
	for (InterfaceCoAList::iterator it=interfaceCoAList.begin(); it!=interfaceCoAList.end(); ++it)
	{
		...
		std::cout << "Cancelling timers for ifID=" << it->first << " and CoA=" << it->second << std::endl;
...
void xMIPv6::createBRRTimer(const IPv6Address& brDest, InterfaceEntry* ie, const uint scheduledTime)
{
	...
	inet::cMessage *brTriggerMsg = new inet::cMessage("sendPeriodicBRR", MK_SEND_PERIODIC_BR);
	...
	if ( pos != transmitIfList.end() )
	{
		...
		if (dynamic_cast<BRTransmitIfEntry*>(pos->second) )
		{
			...
			throw inet::cException("Expected BRTransmitIfEntry* !");
	}
	else
	{
		// we do not yet have an entry -> create a new one
	...
	scheduleAt(inet::simTime()+scheduledTime, brTriggerMsg);
	std::cout<<"\n++++++++++BRR TIMER CREATED FOR SIM TIME: "<<inet::simTime()+scheduledTime<<" seconds+++++++++++++++++ \n";
}


void xMIPv6::sendPeriodicBRR(inet::cMessage *msg)
{
	std::cout <<"\n<<====== Periodic BRR MESSAGE at time: "<<inet::simTime()<<" seconds =====>>\n";
	...
	if ( brIfEntry->retries < BRR_RETRIES ) // we'll not retransmit infinite times
	{
		...
		scheduleAt(inet::simTime() + BRR_TIMEOUT_THRESHOLD, msg);
	}
	else
	{
...
void xMIPv6::processBRRMessage(BindingRefreshRequest* brr, IPv6ControlInfo* ctrlInfo)
{
	...
	{
		std::cout << "BRR not accepted - no binding for this CN. Dropping message." << std::endl;
	}
	else
	{
		InterfaceEntry *ie = ift->getInterfaceById( ctrlInfo->getInterfaceId() );
...
void xMIPv6::createBULEntryExpiryTimer(BindingUpdateList::BindingUpdateListEntry* entry, InterfaceEntry* ie, inet::simtime_t scheduledTime)
{
	//Enter_Method("createBULEntryExpiryTimer()");
	//std::cout << "Creating BUL entry expiry timer for sim time: " << entry->bindingExpiry <<" seconds." << std::endl;
	inet::cMessage* bulExpiryMsg = new inet::cMessage("BULEntryExpiry", MK_BUL_EXPIRY);
	...
	std::cout << "Scheduled BUL expiry (" << entry->bindingExpiry << "s) for time " << scheduledTime << "s" << std::endl;
	// WAS SCHEDULED FOR EXPIRY, NOT 2 SECONDS BEFORE!?!?!?
}


/*BULExpiryIfEntry* xMIPv6::createBULEntryExpiryTimer(Key& key, IPv6Adress& dest, IPv6Adress& HoA, IPv6Adress& CoA, InterfaceEntry* ie, inet::cMessage* bulExpiryMsg)
{
	BULExpiryIfEntry* bulExpIfEntry = (BULExpiryIfEntry*) getTimerIfEntry(key, EXPIRY_TYPE_BUL);
...
void xMIPv6::handleBULExpiry(inet::cMessage* msg)
{
	/*11.7.1
	...
	if ( inet::simTime() < entry->bindingExpiry )
	{
		std::cout << "BUL entry about to expire - creating new BU timer..." << std::endl;
		...
		std::cout << "Resetting expiry timer... " << std::endl;
	...
	{
		std::cout << "BUL entry has expired - removing entry and associated structures..." << std::endl;
...
void xMIPv6::createBCEntryExpiryTimer(IPv6Address& HoA, InterfaceEntry* ie, inet::simtime_t scheduledTime)
{
	inet::cMessage* bcExpiryMsg = new inet::cMessage("BCEntryExpiry", MK_BC_EXPIRY);
	...
	std::cout << "Scheduled BC expiry for time " << scheduledTime << "s" << std::endl;
}


void xMIPv6::handleBCExpiry(inet::cMessage* msg)
{
	/*10.3.1
	...
	std::cout << "BC entry has expired - removing entry and associated structures..." << std::endl;
...
void xMIPv6::createTokenEntryExpiryTimer(IPv6Address& cnAddr, InterfaceEntry* ie, inet::simtime_t scheduledTime, int tokenType)
{
	inet::cMessage* tokenExpiryMsg = new inet::cMessage("TokenEntryExpiry", MK_TOKEN_EXPIRY);
	...
	std::cout << "Scheduled token expiry for time " << scheduledTime << "s" << std::endl;
}


void xMIPv6::handleTokenExpiry(inet::cMessage* msg)
{
	...
	{
		std::cout << "Care-of keygen token for CN=" << tokenExpIfEntry->cnAddr << " expired";
	...
	{
		std::cout << "Home keygen token for CN=" << tokenExpIfEntry->cnAddr << " expired" ;
		bul->resetHomeToken( tokenExpIfEntry->cnAddr, tokenExpIfEntry->ifEntry->ipv6Data()->getMNHomeAddress() );
	}
	else
		throw inet::cException("Unkown value for tokenType!");

	std::cout << "...removed token from BUL." << std::endl;
