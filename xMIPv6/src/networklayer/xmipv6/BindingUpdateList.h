...
class INET_API BindingUpdateList : public inet::cSimpleModule
{
  public:
//###########################Declaration of BUL and BC added by Zarrar Yousaf @ CNI Uni Dortmund on 04.06.07######  
	...
	struct BindingUpdateListEntry
    {
		...
		inet::simtime_t bindingExpiry; // the time at which the lifetime of the binding expires
		...
		inet::simtime_t sentTime; // the time at which that particular BU was sent. recorded from simTime(). Used to implement rate limiting restrcition for sending BU.
		...
		inet::simtime_t sentHoTI, sentCoTI;
  ...
  public:
	 ...
	 */
	virtual void addOrUpdateBUL(const IPv6Address& dest, const IPv6Address& hoa, const IPv6Address& coa, const uint lifetime, const uint seq, const inet::simtime_t buSentTime);//,const simtime_t& nextBUSentTime );
	 ...
	 */
	virtual void addOrUpdateBUL(const IPv6Address& dest, const IPv6Address& hoa, inet::simtime_t sentTime, int cookie, bool isHoTI); // BU for HoTI/CoTI
	 ...
	 */
	virtual MobilityState getMobilityState(const IPv6Address& dest);
...
//
	 ...
	 * destination address.
	 */
	virtual bool isInBindingUpdateList(const IPv6Address& dest); // 10.9.07 - CB
