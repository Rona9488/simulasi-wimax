...
void BindingUpdateList::handleMessage(inet::cMessage *msg)
{
    throw inet::cException("This module doesn't process messages");
}


void BindingUpdateList::addOrUpdateBUL(const IPv6Address& dest, const IPv6Address& hoa, const IPv6Address& coa, const uint lifetime, const uint seq, const inet::simtime_t buSentTime)//,const inet::simtime_t& nextBUSentTime)
{
	...
	entry->bindingExpiry = inet::simTime() + lifetime; // binding expires at this point in time
...
void BindingUpdateList::addOrUpdateBUL(const IPv6Address& dest, const IPv6Address& addr, const inet::simtime_t sentTime, const int cookie, bool HoTI = false)
{
...
int BindingUpdateList::generateBAuthData(const IPv6Address& dest, const IPv6Address& CoA)
{
	...
	if ( entry == NULL )
	{
		std::cout << "Impossible to generate Binding Authorization Data as CN is not existing in BUL!\n";
		return 0;
	}
...
bool BindingUpdateList::isHomeTokenAvailable(const IPv6Address& dest, InterfaceEntry* ie)
{
	...
	return entry->tokenH != UNDEFINED_TOKEN &&
		   (entry->sentHoTI + ie->ipv6Data()->_maxTokenLifeTime()) > inet::simTime();
}
...
bool BindingUpdateList::isCareOfTokenAvailable(const IPv6Address& dest, InterfaceEntry* ie)
{
	...
	return entry->tokenC != UNDEFINED_TOKEN &&
		   (entry->sentCoTI + ie->ipv6Data()->_maxTokenLifeTime()) > inet::simTime();
}
...
bool BindingUpdateList::isValidBinding(const IPv6Address& dest)
{
	...
	return entry->BAck && ( entry->bindingLifetime < SIMTIME_DBL(inet::simTime()) );
}
...
bool BindingUpdateList::isBindingAboutToExpire(const IPv6Address& dest)
{
	...
	return entry->bindingLifetime < SIMTIME_DBL(inet::simTime()) - PRE_BINDING_EXPIRY;
}
...
bool BindingUpdateList::recentlySentCOTI(const IPv6Address& dest, InterfaceEntry* ie)
{
	...
	if (entry->sentCoTI==0.0f)
		return false;
	return entry->sentCoTI + ie->ipv6Data()->_maxTokenLifeTime() / 3 > inet::simTime();
}
...
bool BindingUpdateList::recentlySentHOTI(const IPv6Address& dest, InterfaceEntry* ie)
{
	...
	if (entry->sentHoTI==0.0f)
		return false;
	return entry->sentHoTI + ie->ipv6Data()->_maxTokenLifeTime() / 3 > inet::simTime();
}
