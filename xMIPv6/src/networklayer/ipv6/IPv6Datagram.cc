...
{
    throw inet::cRuntimeError(this, "setExtensionHeaderArraySize() not supported, use addExtensionHeader()");
}
...
{
    throw inet::cRuntimeError(this, "setExtensionHeader() not supported, use addExtensionHeader()");
}
...
IPProtocolId IPv6ExtensionHeader::getExtensionType() const
{
    ...
    if (dynamic_cast<const IPv6HopByHopOptionsHeader*>(this)) {
        ...
        throw inet::cRuntimeError("unrecognised HeaderExtension subclass %s in IPv6ExtensionHeader::getExtensionType()", getClassName());
    }
}
...
inet::cPacket* IPv6Datagram::getEncapsulatedMsg() const
{
    // Kembalikan pesan yang terbungkus jika ada
    return encapsulatedMsg;
}

void IPv6Datagram::setEncapsulatedMsg(inet::cPacket *msg)
{
    // Tentukan pesan yang terbungkus
    encapsulatedMsg = msg;
}
