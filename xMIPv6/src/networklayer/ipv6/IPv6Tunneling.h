...
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\networklayer\xmipv6\MobilityHeader.h" // for HAOpt & RH2
 ...
 */
class INET_API IPv6Tunneling : public inet::cSimpleModule
{
	public:
		enum TunnelType
	    {
	...
	protected:
		...
		struct Tunnel
		{
			Tunnel(const IPv6Address& entry = IPv6Address::UNSPECIFIED_ADDRESS, const IPv6Address& exit = IPv6Address::UNSPECIFIED_ADDRESS,
		...
		struct equalTunnel : public std::binary_function<Tunnels::value_type, Tunnels::value_type, bool>
		{
			...
			{
				return (lhs.second.entry == rhs.second.entry) && (lhs.second.exit == rhs.second.exit) &&
	...
	public:
		 ...
		 * destined for destTrigger. Type of tunnel (normal tunnel, mobility related pseudo-tunnel) is determined by first parameter.
	...
	private:
		 ...
		 * (later on this could be exteded to searching for a tunnel that has a prefix matching the provided address).
		 */
