...
std::string IPv6InterfaceData::info() const
{
    ...
    for (int i=0; i<getNumAddresses(); i++)
    {
        if( rt6->isMobileNode() && getAddress(i).isGlobal())
           os << (i?"\t            , ":"\tAddrs:") << getAddress(i)
              ...
              << " prefExpiryTime: " <<(addresses[i].prefExpiryTime==0?"inf":SIMTIME_STR(addresses[i].prefExpiryTime))<<std::endl;
        else
            os << (i?"\t            , ":"\tAddrs:") << getAddress(i)
               << "(" << IPv6Address::scopeName(getAddress(i).getScope())
    ...
    os << " reachableTime=" << hostVars.reachableTime << std::endl;
    ...
    if (rtrVars.advSendAdvertisements)
    {
        ...
        os << " minRtrAdvInt=" << rtrVars.minRtrAdvInterval << std::endl;
...
void IPv6InterfaceData::assignAddress(const IPv6Address& addr, bool tentative,
                                      inet::simtime_t expiryTime, inet::simtime_t prefExpiryTime)
{
...
inet::simtime_t IPv6InterfaceData::generateReachableTime(double MIN_RANDOM_FACTOR,
    double MAX_RANDOM_FACTOR, uint baseReachableTime)
{
    return (MIN_RANDOM_FACTOR +
            (static_cast<double>(rand()) / RAND_MAX) * (MAX_RANDOM_FACTOR - MIN_RANDOM_FACTOR))
           * baseReachableTime;
}

inet::simtime_t IPv6InterfaceData::generateReachableTime()
{
    return (_getMinRandomFactor() +
            (static_cast<double>(rand()) / RAND_MAX) * (_getMaxRandomFactor() - _getMinRandomFactor()))
...
void IPv6InterfaceData::assignAddress(const IPv6Address& addr, bool tentative, inet::simtime_t expiryTime, inet::simtime_t prefExpiryTime, bool hFlag)
{
    addresses.push_back( AddressData() );
    AddressData& a = addresses.back();
    a.address = addr;
...
{
    std::cout << "\n++++++ Updating the Home Network Information \n IPv6InterfaceData::updateHomeNetworkInfo \n";
    // std::cout << "HoA = "<< hoa.str();
    homeInfo.HoA = hoa;
