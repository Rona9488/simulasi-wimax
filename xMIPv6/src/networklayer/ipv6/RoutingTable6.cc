...
void RoutingTable6::parseXMLConfigFile()
{
    ...
    inet::cXMLElement *config = par("routingTableFile");
    for (inet::cXMLElement *child=config->getFirstChild(); child; child = child->getNextSibling())
    {
        //std::cout << "configuring interfaces from XML file." << std::endl;
        //std::cout << "selected element is: " << child->getTagName() << std::endl;
        // we ensure that the selected element is local.
        if (inet::opp_strcmp(child->getTagName(),"local")!=0) continue;
        //ensure that this is the right parent module we are configuring.
        if (inet::opp_strcmp(child->getAttribute("node"),getParentModule()->getFullName())!=0)
            continue;
        //Go one level deeper.
        //child = child->getFirstChild();
        for (inet::cXMLElement *ifTag=child->getFirstChild(); ifTag; ifTag = ifTag->getNextSibling())
        {
            //The next tag should be "interface".
            if (inet::opp_strcmp(ifTag->getTagName(),"interface")==0)
				//	continue;
            {
                    //std::cout << "Getting attribute: name" << std::endl;
            ...
            else if (inet::opp_strcmp(ifTag->getTagName(),"tunnel")==0)
...
void RoutingTable6::handleMessage(inet::cMessage *msg)
{
    throw inet::cException("This module doesn't process messages");
}

void RoutingTable6::receiveChangeNotification(int category, const inet::cObject *details)
{
    if (getSimulation()->getContextType() == inet::CTX_INITIALIZE)
...
static const char *getRequiredAttr(inet::cXMLElement *elem, const char *attrName)
{
    const char *s = elem->getAttribute(attrName);
    if (!s)
        throw inet::cException("element <%s> misses required attribute %s at %s",
...
void RoutingTable6::configureInterfaceFromXML(InterfaceEntry *ie, inet::cXMLElement *cfg)
{
...
*/
    ...
    inet::cXMLElementList prefixList = cfg->getElementsByTagName("AdvPrefix");
    for (unsigned int i=0; i<prefixList.size(); i++)
    {
        inet::cXMLElement *node = prefixList[i];
        ...
        if (!prefix.prefix.tryParseAddrWithPrefix(node->getNodeValue(),pfxLen))
            throw inet::cException("element <%s> at %s: wrong IPv6Address/prefix syntax %s",
    ...
    // parse addresses
    inet::cXMLElementList addrList = cfg->getChildrenByTagName("inetAddr");
    for (unsigned int k=0; k<addrList.size(); k++)
    {
        inet::cXMLElement *node = addrList[k];
...
void RoutingTable6::configureTunnelFromXML(inet::cXMLElement* cfg)
{
	IPv6Tunneling* tunneling = IPv6TunnelingAccess().get();

	// parse basic config (attributes)
    inet::cXMLElementList tunnelList = cfg->getElementsByTagName("tunnelEntry");
    for (unsigned int i=0; i<tunnelList.size(); i++)
    {
        inet::cXMLElement *node = tunnelList[i];

        IPv6Address entry, exit, trigger;
        entry.set( getRequiredAttr(node, "entryPoint") );
        exit.set( getRequiredAttr(node, "exitPoint") );

        inet::cXMLElementList triggerList = node->getElementsByTagName("triggers");

        if (triggerList.size() != 1)
				throw inet::cException("element <%s> at %s: Only exactly one trigger allowed",
                      node->getTagName(), node->getSourceLocation());

			inet::cXMLElement *triggerNode = triggerList[0];
			trigger.set( getRequiredAttr(triggerNode, "destination") );

			std::cout << "New tunnel: " << "entry=" << entry << ",exit=" << exit << ",trigger=" << trigger << std::endl;
...
const IPv6Route *RoutingTable6::doLongestPrefixMatch(const IPv6Address& dest)
{
    ...
    while ( it!=routeList.end() ){
    	if ( dest.matches((*it)->getDestPrefix(),(*it)->getPrefixLength()) ){
    		if (inet::simTime() > (*it)->getExpiryTime() && (*it)->getExpiryTime() != 0){//since 0 represents infinity.
    			if ( (*it)->getSrc()==IPv6Route::FROM_RA ){
                std::cout << "Expired prefix detected!!" << std::endl;
...
{
    std::cout << "// adding route: " << *route << std::endl; // Added by CB
...
{
	std::cout << "/// Removing default route for interface=" << interfaceID << std::endl;
...
{
	std::cout << "/// Removing all routes from rt6 " << std::endl;
