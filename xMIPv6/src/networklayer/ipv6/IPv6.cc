...
void IPv6::endService(inet::cPacket *msg)
{
		...
		if ( sDgram->ie->ipv6Data()->isTentativeAddress(sDgram->datagram->getSrcAddress()) )
    {
	   ...
	   (msg->getArrivalGate()->isName("ndIn") && dynamic_cast<inet::IPv6NDMessage*>(msg)) ||
	   (msg->getArrivalGate()->isName("icmpIn") && dynamic_cast<inet::ICMPv6Message*>(msg)) || //Added this for ICMP msgs from ICMP module-WEI
...
InterfaceEntry *IPv6::getSourceInterfaceFrom(inet::cPacket *msg)
{
...
void IPv6::handleMessageFromHL(inet::cPacket *msg)
{
    ...
    if (datagram == NULL)
    {
        std::cout << "Encapsulation failed - dropping packet." << endl;
    	delete msg;
    	return;
    }
...
*/
    std::cout << "fragmentation not implemented yet\n";

    // route packet
    if (destIE!=NULL)
    {
        std::cout << "fragmentAndRoute: sending immediately to output." << endl;
...
void IPv6::routePacket(IPv6Datagram *datagram, InterfaceEntry *destIE, bool fromHL)
{
    ...
    std::cout << "Routing datagram '" << datagram->getName() << "' with dest=" << destAddress << ":\n";

    // local delivery of unicast packets
    if (rt->isLocalAddress(destAddress))
    {
        std::cout << "local delivery\n";
    ...
    if (!fromHL)
    {
        ...
        {
            std::cout << "forwarding is off, dropping packet\n";
            numDropped++;
            delete datagram;
            return;
        }
        ...
        {
            std::cout << "dest address is link-local (or weaker) scope, doesn't get forwarded\n";
            delete datagram;
            return;
        }
	...
	if ( interfaceId > ift->getNumInterfaces() )
	    ...
	    std::cout << "tunneling: src addr=" << datagram->getSrcAddress() << ", dest addr=" << destAddress << std::endl;
    ...
    ASSERT(ie!=NULL);
    std::cout << "next hop for " << destAddress << " is " << nextHop << ", interface " << ie->getName() << "\n";
    ASSERT(!nextHop.isUnspecified() && ie!=NULL);
	 ...
	 if ( rt->isMobileNode() )
	 {
		 ...
		 {
		     std::cout << "Using HoA instead of CoA... dropping datagram" << endl;
			 delete datagram;
			 numDropped++;
			 return;
		 }
    ...
    {
        std::cout << "no link-layer address for next hop yet, passing datagram to Neighbour Discovery module\n";
        send(datagram, "ndOut");
        return;
    }
    std::cout << "link-layer address: " << macAddr << "\n";
    ...
    if (datagram->getSrcAddress().isUnspecified())
    {
        ...
        {
            std::cout << "Source address is tentative - enqueueing datagram for later resubmission." << endl;
...
void IPv6::routeMulticastPacket(IPv6Datagram *datagram, InterfaceEntry *destIE, InterfaceEntry *fromIE)
{
    ...
    std::cout << "destination address " << destAddr << " is multicast, doing multicast routing\n";
    numMulticast++;
    ...
    if (fromIE!=NULL)
    {
        ...
        {
            std::cout << "local delivery of multicast packet\n";
            numLocalDeliver++;
        ...
        {
            std::cout << "multicast dest address is link-local (or smaller) scope\n";
            delete datagram;
            return;
        }
    ...
    std::cout << "sending out datagram on every interface (except incoming one)\n";
...
*/
    ...
    inet::cPacket *packet = decapsulate(datagram);

    if (protocol==IP_PROT_IPv6_ICMP && dynamic_cast<inet::IPv6NDMessage*>(packet))
    {
        std::cout << "Neigbour Discovery packet: passing it to ND module\n";
        send(packet, "ndOut");
    }
    else if (protocol==IP_PROT_IPv6EXT_MOB && dynamic_cast<MobilityHeader*>(packet))
    {
    	...
    	{
    		std::cout << "MIPv6 packet: passing it to xMIPv6 module\n";
    	...
    	else
    	{
    		...
    		std::cout << "No MIPv6 support on this node!\n";
    		IPv6ControlInfo *ctrlInfo = check_and_cast<IPv6ControlInfo*>(packet->removeControlInfo());
    		icmp->sendErrorMessage(packet, ctrlInfo, inet::ICMPv6_PARAMETER_PROBLEM, 1);

    		//delete packet; // 13.9.07 - CB, update 21.9.07 - CB
    	}
    }
    else if (protocol==IP_PROT_IPv6_ICMP && dynamic_cast<inet::ICMPv6Message*>(packet))
    {
        std::cout << "ICMPv6 packet: passing it to ICMPv6 module\n";
        send(packet, "icmpOut");
    }//Added by WEI to forward ICMPv6 msgs to ICMPv6 module.
    else if (protocol==IP_PROT_IP || protocol==IP_PROT_IPv6)
    {
        std::cout << "Tunnelled IP datagram\n";
    ...
    else
    {
        ...
        // 21.9.07 - CB
        inet::cGate* outGate = gate("transportOut", gateindex);
        if (! outGate->isConnected() )
        {
        	std::cout << "Transport layer gate not connected - dropping packet!\n";
        	delete packet;
        }
        else
        {
            std::cout << "Protocol " << protocol << ", passing up on gate " << gateindex << "\n";
...
void IPv6::handleReceivedICMP(inet::cMessage *msg) {
    std::cout << "\n<<=======THIS IS THE IPv6::handleReceivedICMP() FUNCTION=========>>\n";

    inet::ICMPv6Message *icmpMsg = dynamic_cast<inet::ICMPv6Message *>(msg);
    if (!icmpMsg) {
        std::cerr  << "Received a non-ICMPv6Message in handleReceivedICMP(). Discarding.\n";
        delete msg;
        return;
    }

    switch (icmpMsg->getType()) {
        case inet::ICMPv6_REDIRECT:
        case inet::ICMPv6_DESTINATION_UNREACHABLE:
        case inet::ICMPv6_PACKET_TOO_BIG:
        case inet::ICMPv6_TIME_EXCEEDED:
        case inet::ICMPv6_PARAMETER_PROBLEM: {
            // Error handling
            IPv6Datagram *bogusPacket = check_and_cast<IPv6Datagram *>(icmpMsg->getObject("encapsulatedMessage"));
            if (!bogusPacket) {
                std::cerr  << "Missing encapsulatedMessage in ICMPv6Message.\n";
                delete msg;
                return;
            }

            int protocol = bogusPacket->getTransportProtocol();
            const char *gateName = "transportOut";
            if (gate(gateName)) {
                int gateindex = gate(gateName)->isVector() ? protocol : 0; // Mapping example
                send(msg, gateName, gateindex);
            } else {
                std::cerr  << "Gate '" << gateName << "' not found. Dropping ICMP message.\n";
                delete msg;
            }
            break;
        }
        ...
        default: {
            ...
            const char *gateName = "transportOut";
            if (gate(gateName)) {
                int gateindex = gate(gateName)->isVector() ? IP_PROT_ICMP : 0;
                send(msg, gateName, gateindex);
            } else {
                std::cerr  << "Gate '" << gateName << "' not found. Dropping ICMP message.\n";
                delete msg;
...
inet::cPacket *IPv6::decapsulate(IPv6Datagram *datagram)
{
    // decapsulate transport packet
    InterfaceEntry *fromIE = getSourceInterfaceFrom(datagram);
    inet::cPacket *packet = datagram->decapsulate();
...
IPv6Datagram *IPv6::encapsulate(inet::cPacket *transportPacket, InterfaceEntry *&destIE)
{
std::cout <<"\n<<=======THIS IS THE IPv6::encapsulate() FUNCTION=========>>\n";
    ...
    for (int i = 0; i < controlInfo->extensionHeaderArraySize(); i++)
	{
    	...
    	std::cout << "Copied extension header to datagram." << std::endl;
	}
...
void IPv6::sendDatagramToOutput(IPv6Datagram *datagram, InterfaceEntry *ie, const MACAddress& macAddr)
{
    ...
    if (datagram->getHopLimit() <= 0)
    {
        ...
        std::cout << "datagram hopLimit reached zero, sending ICMPv6_TIME_EXCEEDED\n";
        icmp->sendErrorMessage(datagram, inet::ICMPv6_TIME_EXCEEDED, 0); // FIXME check icmp 'code'
        return;
    }
...
bool IPv6::determineOutputInterface(const IPv6Address& destAddress, IPv6Address& nextHop, int& interfaceId, IPv6Datagram* datagram)
{
    ...
    if (interfaceId==-1)
    {
        ...
        std::cout << "do longest prefix match in routing table" << std::endl;
    	const IPv6Route *route = rt->doLongestPrefixMatch(destAddress);
    	std::cout << "finished longest prefix match in routing table" << std::endl;
        if (!route)
        {
            if (rt->isRouter())
            {
                std::cout << "unroutable, sending ICMPv6_DESTINATION_UNREACHABLE\n";
                numUnroutable++;
                icmp->sendErrorMessage(datagram, inet::ICMPv6_DESTINATION_UNREACHABLE, 0); // FIXME check ICMP 'code'
            }
            else // host
            {
                std::cout << "no match in routing table, passing datagram to Neighbour Discovery module for default router selection\n";
                send(datagram, "ndOut");
            }
            return false;
        }
...
bool IPv6::processExtensionHeaders(IPv6Datagram* datagram)
{
	...
	std::cout << noExtHeaders << " extension header(s) for processing..." << std::endl;
	...
	for (int i = 0; i < noExtHeaders; i++)
	{
		...
		if ( dynamic_cast<IPv6RoutingHeader*>(eh) )
		{
			...
			std::cout << "Routing Header with type=" << rh->getRoutingType() << std::endl;
			...
			if ( rt->hasMIPv6Support() && rh->getRoutingType() == 2 )
			{
				...
				std::cout << "Sending datagram with RH2 to MIPv6 module" << std::endl;
			...
			{
				delete rh;
				std::cout << "Ignoring unknown routing header" << std::endl;
			}
}
		else if ( dynamic_cast<IPv6DestinationOptionsHeader*>(eh) )
		{
			...
			if ( rt->hasMIPv6Support() && dynamic_cast<HomeAddressOption*>(eh) )
			{
				...
				std::cout << "Sending datagram with HoA Option to MIPv6 module" << std::endl;
				send(datagram, "xMIPv6Out");
				return false;
			}
			else
			{
				delete eh;
				std::cout << "Ignoring unknown destination options header" << std::endl;
			}
		}
		else
		{
			delete eh;
			std::cout << "Ignoring unknown extension header" << std::endl;
		}
	}
