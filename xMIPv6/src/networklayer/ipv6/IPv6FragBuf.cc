...
IPv6Datagram *IPv6FragBuf::addFragment(IPv6Datagram *datagram, IPv6FragmentHeader *fh, inet::simtime_t now)
{
...
void IPv6FragBuf::purgeStaleFragments(inet::simtime_t lastupdate)
{
    ...
    for (Buffers::iterator i=bufs.begin(); i!=bufs.end(); )
    {
        ...
        {
            // send ICMP error
            std::cout << "datagram fragment timed out in reassembly buffer, sending ICMP_TIME_EXCEEDED\n";
            icmpModule->sendErrorMessage(buf.datagram, inet::ICMPv6_TIME_EXCEEDED, 0);
