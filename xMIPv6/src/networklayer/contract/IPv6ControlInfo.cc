...
{
    if (dgram)
        throw inet::cException("IPv6ControlInfo::setOrigDatagram(): a datagram is already attached");
    dgram = d;
    take(dgram);
}
...
{
    if (!dgram)
        throw inet::cException("IPv6ControlInfo::removeOrigDatagram(): no datagram attached "
