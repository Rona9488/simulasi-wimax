...
inline void doPacking(inet::cCommBuffer *buf, const IPAddress& addr)
{
    buf->pack(addr.getInt());
}

inline void doUnpacking(inet::cCommBuffer *buf, IPAddress& addr)
{
    int32_t d; buf->unpack(d); addr.set(d);
}
