...
class INET_API IPAddressResolver
{
  ...
  public:
     ...
     */
    virtual IPvXAddress addressOf(inet::cModule *host, int addrType=ADDR_PREFER_IPv6);

    /**
     * Similar to addressOf(), but only looks at the given interface
     */
    virtual IPvXAddress addressOf(inet::cModule *host, const char *ifname, int addrType=ADDR_PREFER_IPv6);
     ...
     */
    virtual IPAddress routerIdOf(inet::cModule *host);
     ...
     */
    virtual IInterfaceTable *interfaceTableOf(inet::cModule *host);
     ...
     * the host/router module. Throws an error if not found.
     */
    virtual IRoutingTable *routingTableOf(inet::cModule *host);
     ...
     * the host/router module. Throws an error if not found.
     */
    virtual RoutingTable6 *routingTable6Of(inet::cModule *host);
#endif
     ...
     * if not found.
     */
    virtual NotificationBoard *notificationBoardOf(inet::cModule *host);

    /**
     * Like interfaceTableOf(), but doesn't throw error if not found.
     */
    virtual IInterfaceTable *findInterfaceTableOf(inet::cModule *host);

    /**
     * Like routingTableOf(), but doesn't throw error if not found.
     */
    virtual IRoutingTable *findRoutingTableOf(inet::cModule *host);

#ifndef WITHOUT_IPv6
    /**
     * Like interfaceTableOf(), but doesn't throw error if not found.
     */
    virtual RoutingTable6 *findRoutingTable6Of(inet::cModule *host);
#endif

    /**
     * Like notificationBoardOf(), but doesn't throw error if not found.
     */
    virtual NotificationBoard *findNotificationBoardOf(inet::cModule *host);
    //@}
};
