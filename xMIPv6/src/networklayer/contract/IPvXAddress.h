...
class INET_API IPvXAddress
{
  ...
  public:
    ...
    IPAddress get4() const {
        if (isv6)
            throw inet::cRuntimeError("IPvXAddress: cannot return IPv6 address %s as IPv4", str().c_str());
    ...
    IPv6Address get6() const {
        if (!isv6)  {
            if (d[0]==0) // allow null address to be returned as IPv6
                return IPv6Address();
            throw inet::cRuntimeError("IPvXAddress: cannot return IPv4 address %s as IPv6", str().c_str());
        }
    ...
    void set(const IPv6Address& addr)  {
        ...
        isv6 = true;
        uint32_t *w = const_cast<IPv6Address&>(addr).words();
    ...
    void set(const char *addr) {
        if (!tryParse(addr))
            throw inet::cRuntimeError("IPvXAddress: cannot interpret address string `%s'", addr);
    }
...
inline void doPacking(inet::cCommBuffer *buf, const IPvXAddress& addr)
{
...
inline void doUnpacking(inet::cCommBuffer *buf, IPvXAddress& addr)
{
    if (buf->checkFlag()) {
        IPv6Address tmp;
        doUnpacking(buf, tmp);
