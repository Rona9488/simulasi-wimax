...
void MPLS::handleMessage(inet::cMessage * msg)
{
    if (!strcmp(msg->getArrivalGate()->getName(), "ifIn"))
    {
        std::cout << "Processing message from L2: " << msg << std::endl;
        processPacketFromL2(msg);
    }
    else if (!strcmp(msg->getArrivalGate()->getName(), "netwIn"))
    {
        std::cout << "Processing message from L3: " << msg << std::endl;
...
void MPLS::sendToL2(inet::cMessage *msg, int gateIndex)
{
    send(msg, "ifOut", gateIndex);
}

void MPLS::processPacketFromL3(inet::cMessage * msg)
{
    ...
    {
        TCPSegment *seg = check_and_cast<TCPSegment*>(ipdatagram->getObject("encapsulatedMsg"));
...
bool MPLS::tryLabelAndForwardIPDatagram(IPDatagram *ipdatagram)
{
    ...
    {
        std::cout << "no mapping exists for this packet" << std::endl;
        return false;
    }
    ...
    std::cout << "forwarding packet to " << outInterface << std::endl;
...
void MPLS::labelAndForwardIPDatagram(IPDatagram *ipdatagram)
{
    ...
    std::cout << "FEC not resolved, doing regular L3 routing" << std::endl;
...
void MPLS::doStackOps(MPLSPacket *mplsPacket, const LabelOpVector& outLabel)
{
    ...
    std::cout << "doStackOps: " << outLabel << std::endl;
...
void MPLS::processPacketFromL2(inet::cMessage *msg)
{
...
void MPLS::processMPLSPacketFromL2(MPLSPacket *mplsPacket)
{
    ...
    std::cout << "Received " << mplsPacket << " from L2, label=" << oldLabel << " inInterface=" << senderInterface << std::endl;

    if (oldLabel==-1)
    {
        // This is a IP native packet (RSVP/TED traffic)
        // Decapsulate the message and pass up to L3
        std::cout << ": decapsulating and sending up\n";
    ...
    if (!found)
    {
        std::cout << "discarding packet, incoming label not resolved" << std::endl;
    ...
    else
    {
        ...
        std::cout << "decapsulating IP datagram" << std::endl;
