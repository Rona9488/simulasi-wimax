...
void LIBTable::handleMessage(inet::cMessage *)
{
    ASSERT(false);
}
...
void LIBTable::readTableFromXML(const inet::cXMLElement* libtable)
{
    ASSERT(libtable);
    ASSERT(!strcmp(libtable->getTagName(), "libtable"));
    checkTags(libtable, "libentry");
    inet::cXMLElementList list = libtable->getChildrenByTagName("libentry");
    for (inet::cXMLElementList::iterator it=list.begin(); it != list.end(); it++)
    {
        const inet::cXMLElement& entry = **it;
        ...
        inet::cXMLElementList ops = getUniqueChild(&entry, "outLabel")->getChildrenByTagName("op");
        for (inet::cXMLElementList::iterator oit=ops.begin(); oit != ops.end(); oit++)
        {
            const inet::cXMLElement& op = **oit;
