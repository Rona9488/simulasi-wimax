...
std::ostream& operator<<(std::ostream& os, const NotificationBoard::NotifiableVector& v)
{
    ...
    {
        os << (i==0 ? ": " : ", ");
        if (dynamic_cast<inet::cModule*>(v[i]))
        {
            inet::cModule *mod = dynamic_cast<inet::cModule*>(v[i]);
            os << "mod (" << mod->getClassName() << ")" << mod->getFullName() << " id=" << mod->getId();
        }
        else if (dynamic_cast<inet::cObject*>(v[i]))
        {
            inet::cObject *obj = dynamic_cast<inet::cObject*>(v[i]);
            os << "a " << obj->getClassName();
        }
        else
        {
            os << "a " << inet::opp_typename(typeid(v[i]));
        }
    }
    return os;
}
...
void NotificationBoard::handleMessage(inet::cMessage *msg)
{
    throw inet::cException("NotificationBoard doesn't handle messages, it can be accessed via direct method calls");
}
...
void NotificationBoard::fireChangeNotification(int category, const inet::cObject *details)
{
