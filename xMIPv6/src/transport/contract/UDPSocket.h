 ...
 * }
 *
 * void MyModule::handleMessage(inet::cMessage *msg) {
 ...
 * }
 *
 * void MyModule::socketDatagramArrived(int, void *, inet::cMessage *msg, UDPControlInfo *ctrl) {
...
class INET_API UDPSocket
{
  public:
    ...
    class CallbackInterface
    {
      public:
        ...
        virtual void socketDatagramArrived(int sockId, void *yourPtr, inet::cMessage *msg, UDPControlInfo *ctrl) = 0;
  ...
  protected:
    ...
    inet::cGate *gateToUdp;

  protected:
    void sendToUDP(inet::cMessage *msg);
  ...
  public:
     ...
     */
    void setOutputGate(inet::cGate *toUdp)  {gateToUdp = toUdp;}
     ...
     */
    void sendTo(inet::cMessage *msg, IPvXAddress destAddr, int destPort);

    /**
     * Sends a data packet to the address and port specified previously
     * in a connect() call.
     */
    void send(inet::cMessage *msg);
     ...
     * that of the socket.)
     */
    bool belongsToSocket(inet::cMessage *msg);
     ...
     */
    static bool belongsToAnyUDPSocket(inet::cMessage *msg);
     ...
     * return true!
     */
    void processMessage(inet::cMessage *msg);
    //@}
};
