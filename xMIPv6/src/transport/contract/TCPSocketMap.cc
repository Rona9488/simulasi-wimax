...
TCPSocket *TCPSocketMap::findSocketFor(inet::cMessage *msg)
{
    TCPCommand *ind = dynamic_cast<TCPCommand *>(msg->getControlInfo());
    if (!ind)
        throw inet::cException("TCPSocketMap: findSocketFor(): no TCPCommand control info in message (not from TCP?)");
