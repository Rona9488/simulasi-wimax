...
void TCP::initialize()
{
    ...
    inet::cModule *netw = getParentModule();  // Get the parent module instead of using simulation.getSystemModule()
...
void TCP::handleMessage(inet::cMessage *msg)
{
...
{
    bool disable_tracing = getenv("DISABLE_TRACING") != nullptr;
    if (disable_tracing) {
...
void TCP::addForkedConnection(TCPConnection *conn, TCPConnection *newConn, IPvXAddress localAddr, IPvXAddress remoteAddr, int localPort, int remotePort)
{
    ...
    // Generate a unique connId for conn using a static counter
    static int uniqueCounter = 0;
    key.connId = conn->connId = ++uniqueCounter;
    tcpAppConnMap[key] = conn;
