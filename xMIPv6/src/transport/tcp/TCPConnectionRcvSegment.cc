...
//        uint32_t ackNo = tcpseg->getSequenceNo() + (uint32_t)tcpseg->getPayloadLength();
...
//            // section 2.5).
//                // uint32_t old_rcv_nxt = state->rcv_nxt; // The same has already be done in line 401 - FIXED 2009-08-05 by T.R.
//
...
//        uint32_t fin_seq = (uint32_t)tcpseg->getSequenceNo() + (uint32_t)tcpseg->getPayloadLength();
...
//            throw inet::cException("Internal error: received CONN_ESTAB timeout in state %s", stateName(fsm.getState()));
//    }
//}
//
...
//        default:
//            // We should not receive this timeout in this state.
//            throw inet::cException("Internal error: received time-wait (2MSL) timeout in state %s", stateName(fsm.getState()));
//    }
//
//}
//
...
//        default:
//            // We should not receive this timeout in this state.
//            throw inet::cException("Internal error: received FIN_WAIT_2 timeout in state %s", stateName(fsm.getState()));
//    }
//}
//
...
//        default:  throw inet::cException("Internal error: SYN-REXMIT timer expired while in state %s", stateName(fsm.getState()));
//    }
//
//    // reschedule timer
