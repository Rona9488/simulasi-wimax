...
//#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\transport\contract\TCPCommand_m.h"
...
//    conn->sendQueue = check_and_cast<TCPSendQueue *>(inet::createOne(sendQueueClass));
//    conn->sendQueue->setConnection(conn);
//
//    const char *receiveQueueClass = receiveQueue->getClassName();
//    conn->receiveQueue = check_and_cast<TCPReceiveQueue *>(inet::createOne(receiveQueueClass));
...
//    conn->tcpAlgorithm = check_and_cast<TCPAlgorithm *>(inet::createOne(tcpAlgorithmClass));
...
//        tcpseg->setControlInfo(controlInfo);
//
//        check_and_cast<TCP *>(inet::getSimulation()->getSystemModule())->send(tcpseg,"ipOut");
//    }
//    else
//    {
//        // send over IPv6
...
//        tcpseg->setControlInfo(controlInfo);
//
//        check_and_cast<TCP *>(inet::getSimulation()->getSystemModule())->send(tcpseg,"ipv6Out");
//    }
//}
//
...
//    inet::cMessage *msg = new inet::cMessage(indicationName(code));
//    msg->setKind(code);
...
//    inet::cMessage *msg = new inet::cMessage(indicationName(TCP_I_ESTABLISHED));
...
//}
//
//void TCPConnection::sendToApp(inet::cMessage *msg)
//{
//    tcpMain->send(msg, "appOut", appGateIndex);
//}
//
...
//    sendQueue = check_and_cast<TCPSendQueue *>(inet::createOne(sendQueueClass));
...
//    receiveQueue = check_and_cast<TCPReceiveQueue *>(inet::createOne(receiveQueueClass));
...
//    tcpAlgorithm = check_and_cast<TCPAlgorithm *>(inet::createOne(tcpAlgorithmClass));
...
//        throw inet::cRuntimeError("Invalid advertisedWindow parameter: %ld", advertisedWindowPar);
//
...
//        {
//            std::cout << "If you want to use TCP SACK please set tcpAlgorithmClass to TCPReno" << std::endl;
//            ASSERT(false);
//        }
//    }
//}
...
//    state->iss = (unsigned long)fmod(SIMTIME_DBL(inet::simTime())*250000.0, 1.0+(double)(unsigned)0xffffffffUL) & 0xffffffffUL;
//
...
//    uint32_t len = tcpseg->getPayloadLength();
//    uint32_t seqNo = tcpseg->getSequenceNo();
//
//    if (len == 0)
//    {
//        if (state->rcv_wnd == 0)
...
//        throw inet::cException("Error processing command OPEN_ACTIVE: foreign socket unspecified");
//    if (localPort==-1)
//        throw inet::cException("Error processing command OPEN_ACTIVE: local port unspecified");
//
//    // create segment
...
//}
//
//void TCPConnection::sendRst(uint32_t seq, IPvXAddress src, IPvXAddress dest, int srcPort, int destPort)
//{
...
//}
//
//void TCPConnection::sendRstAck(uint32_t seq, uint32_t ack, IPvXAddress src, IPvXAddress dest, int srcPort, int destPort)
//{
...
//}
//
//void TCPConnection::sendSegment(uint32_t bytes)
//{
...
//}
//
//bool TCPConnection::sendData(bool fullSegmentsOnly, uint32_t congestionWindow) // changed from int congestionWindow to uint32_t congestionWindow 2009-08-05 by T.R.
//{
//    if (!state->afterRto)
...
//                            state->snd_mss = std::min(state->snd_mss, (uint32_t) option.getValues(0));
//                            if (state->snd_mss==0)
//                                state->snd_mss = 536;
...
//                                count++;
//
//                                uint32_t sack_range = tmp.getEnd() - tmp.getStart();
...
//                                uint32_t tmp_sack_range = sack_range;
//                                uint32_t counter = 1; // at least one piece has been received
//
...
//                                uint32_t mss_sized_sack = tmp.getStart();
...
//    if (state->snd_dsack)
//    {
//        uint32_t start_new = receiveQueue->getLE(start);
//        uint32_t end_new = receiveQueue->getRE(end);
...
//    for (uint32_t s1=state->snd_una; s1<state->snd_max; s1=s1+state->snd_mss) // Note: Would be better use byte ranges
//    {
...
//}
//
//uint32_t TCPConnection::nextSeg()
//{
//    ASSERT (state->sack_enabled);
...
//    for (uint32_t s2=state->snd_una; s2<state->snd_max; s2=s2+state->snd_mss) // Note: Would be better use byte ranges
//    {
...
//    if (!found)
//    {
//        for (uint32_t s3=state->snd_una; s3<state->snd_max; s3=s3+state->snd_mss) // Note: Would be better use byte ranges
//        {
...
//        // steps C.1 -- C.5)."
//        uint32_t seqNum = nextSeg(); // if nextSeg() returns 0 (=failure): terminate steps C.1 -- C.5
//        if (seqNum != 0)
//        {
...
//}
//
//void TCPConnection::sendSegmentDuringLossRecoveryPhase(uint32_t seqNum)
//{
//    ASSERT (state->sack_enabled && state->lossRecovery);
...
//    sendSegment(state->snd_mss);
//
//    uint32_t sentSeqNum = seqNum + state->snd_mss;
//
...
//            {
//                uint32_t effectiveWin = std::min (state->snd_wnd, congestionWindow) - outstandingData + 2*state->snd_mss; // RFC 3042, page 3: "(...)the sender can only send two segments beyond the congestion window (cwnd)."
