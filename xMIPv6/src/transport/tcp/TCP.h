...
class TCPConnection;
class TCPSegment;
bool disable_tracing = false;  // Set to 'true' to disable tracing

// macro for normal ev<< logging (note: deliberately no parens in macro def)
#define tcpEV ((disable_tracing || TCP::testing) ? std::cout : std::cerr)

// macro for more verbose logging
#define tcpEV2 ((disable_tracing || TCP::testing || !TCP::logverbose) ? std::cout : std::cerr)

// testingEV writes log that automated test cases can check (*.test files)
#define testingEV ((disable_tracing || !TCP::testing) ? std::cout : std::cerr)
 ...
 */
class INET_API TCP : public inet::cSimpleModule
{
  public:
