...
//#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\transport\contract\SCTPCommand_m.h"
...
//{
//	nextEphemeralPort = (uint16_t)(rand()%10000 + 30000);
//
//	//sctpEV3<<"SCTPMain initialize\n";
//
//	inet::cModule *netw = getSimulation()->getSystemModule();
//
//	testing = netw->hasPar("testing") && netw->par("testing").boolValue();
//	if (netw->hasPar("testTimeout"))
//	{
//		testTimeout = (inet::simtime_t)netw->par("testTimeout");
//	}
...
//	if (this->hasPar("dataRate") && (int32_t)par("dataRate")>0)
//		dataRate = (int32_t)par("dataRate");
...
//
//
//void SCTP::handleMessage(inet::cMessage *msg)
//{
...
//		{
//			std::cout<<"Packet has bit-error. delete it\n";
//
//			bitError = true;
//			numPacketsDropped++;
//			delete msg;
...
////		int32_t assocId = controlInfo->getAssocId();
...
//{
//    if (inet::getEnvir()->getConfig()->getAsInt("disable_tracing", 0) != 0) {
...
//}
//
//SCTPAssociation *SCTP::findAssocForMessage(IPvXAddress srcAddr, IPvXAddress destAddr, uint32_t srcPort, uint32_t destPort, bool findListen)
//{
//	SockPair key;
//
...
//}
//
//void SCTP::updateSockPair(SCTPAssociation *conn, IPvXAddress localAddr, IPvXAddress remoteAddr, int32_t localPort, int32_t remotePort)
//{
//	SockPair key;
...
//}
//
//void SCTP::addForkedAssociation(SCTPAssociation *assoc, SCTPAssociation *newAssoc, IPvXAddress localAddr, IPvXAddress remoteAddr, int32_t localPort, int32_t remotePort)
//{
//SockPair keyAssoc;
//
//	std::cout<<"addForkedConnection assocId="<<assoc->assocId<<"  newId="<<newAssoc->assocId<<"\n";
//
...
//		{
//		    it->second.stop = getSimulation()->getSimTime();
...
//		removeAssociation(j->second);
//	}
//	std::cout << getFullPath() << ": finishing SCTP with " << sctpConnMap.size() << " connections open.\n";
//
//	for (AssocStatMap::iterator iter=assocStatMap.begin(); iter!=assocStatMap.end(); iter++)
//	{
//	    std::cout<<"Assoc "<<iter->second.assocId<<": started at "<<iter->second.start<<" and finished at "<<iter->second.stop<<" --> lifetime: "<<iter->second.lifeTime<<"\n";
//		recordScalar("lifetime", iter->second.lifeTime);
//		std::cout<<"Assoc "<<iter->second.assocId<<": sent Bytes="<<iter->second.sentBytes<<", acked Bytes="<<iter->second.ackedBytes<<", throughput="<<iter->second.throughput<<" Bit/sec\n";
//		recordScalar("acked Bytes", iter->second.ackedBytes);
//		recordScalar("throughput [Bit/sec]", iter->second.throughput);
//		std::cout<<"Assoc "<<iter->second.assocId<<": transmitted Bytes="<<iter->second.transmittedBytes<<", retransmitted Bytes="<<iter->second.transmittedBytes-iter->second.ackedBytes<<"\n";
//		recordScalar("transmitted Bytes", iter->second.transmittedBytes);
//		std::cout<<"Assoc "<<iter->second.assocId<<": Chunks Scheduled for Fast Retransmission="<<iter->second.numFastRtx<<", No timer based Rtx="<<iter->second.numT3Rtx<<", No path Failures="<<iter->second.numPathFailures<<", No ForwardTsn="<<iter->second.numForwardTsn<<"\n";
