...
////
//
//SCTPAssociation::SCTPAssociation(SCTP *_mod, int32_t _appGateIndex, int32_t _assocId)
//{
//	sctpMain = _mod;
//	appGateIndex = _appGateIndex;
//	assocId = _assocId;
//
//	std::cout<<"SCTPAssociationBase:SCTPAssociation new assocId="<<assocId<<"\n";
//
...
//	T1_InitTimer = new inet::cMessage(timername);
//	sprintf(timername, "T2_SHUTDOWN of assoc %d", assocId);
//	T2_ShutdownTimer = new inet::cMessage(timername);
//	sprintf(timername, "T5_SHUTDOWN_GUARD of assoc %d", assocId);
//	T5_ShutdownGuardTimer = new inet::cMessage(timername);
//	sprintf(timername, "SACK_TIMER of assoc %d", assocId);
//	SackTimer = new inet::cMessage(timername);
//
//	if (sctpMain->testTimeout>0)
//	{
//		StartTesting = new inet::cMessage("StartTesting");
...
//}
//
//bool SCTPAssociation::processTimer(inet::cMessage *msg)
//{
//	SCTPPathVariables* path = NULL;
//
//	sctpEV3 << msg->getName() << " timer expired at " << inet::simTime() << "\n";
//
...
//	{
//	    sctpEV3 << inet::simTime() << " delayed Sack: cTsnAck=" << state->cTsnAck
//	            << " highestTsnReceived=" << state->highestTsnReceived
...
//		throw inet::cException("Unknown message kind in app command");
//                 return (SCTPEventCode)0; // to satisfy compiler
//    }
//}
//
//bool SCTPAssociation::processAppCommand(inet::cPacket *msg)
//{
//	printConnBrief();
//
...
//		default: throw inet::cException("wrong event code");
//	}
//	delete sctpCommand;
//	// then state transitions
//	return performStateTransition(event);
...
//	{
//		std::cout << "Staying in state: " << stateName(fsm->getState()) << " (no FSM event)\n";
//		return true;
//	}
//
//	// state machine
...
//	{
//		std::cout << "Transition: " << stateName(oldState) << " --> " << stateName(fsm->getState()) << "  (event was: " << eventName(event) << ")\n";
//		sctpEV3 << sctpMain->getName() << ": " << stateName(oldState) << " --> " << stateName(fsm->getState()) << "  (on " << eventName(event) << ")\n";
//		stateEntered(fsm->getState());
//	}
//	else
//	{
//		std::cout<< "Staying in state: " << stateName(fsm->getState()) << " (event was: " << eventName(event) << ")\n";
//	}
...
//				state->header = 0;
//			state->swsLimit = (uint32_t)sctpMain->par("swsLimit");
...
//			cumTsnAck = new inet::cOutVector(str);
//			sprintf(str, "SendQueue of assoc %d", assocId);
//			sendQueue = new inet::cOutVector(str);
//			state->sendQueueLimit = (uint32_t)sctpMain->par("sendQueueLimit");
