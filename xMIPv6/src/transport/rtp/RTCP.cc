...
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\transport\contract\UDPSocket.h"
#include "C:\Users\ronaa\Downloads\numbat-master (2)\numbat-master\numbat-master\xMIPv6\src\transport\contract\UDPControlInfo_m.h"
...
void RTCP::initialize()
{
    ...
    _participantInfos = new inet::cArray("ParticipantInfos");
}
...
void RTCP::handleMessage(inet::cMessage *msg)
{
...
void RTCP::handleMessageFromRTP(inet::cMessage *msg)
{
...
void RTCP::handleMessageFromUDP(inet::cMessage *msg)
{
    // from SocketLayer all message are of type inet::cMessage
    readRet(PK(msg));
}


void RTCP::handleSelfMessage(inet::cMessage *msg)
{
...
{
    _senderInfo->setStartTime(inet::simTime());
...
void RTCP::connectRet()
{
    ...
    inet::cMessage *reminderMessage = new inet::cMessage("Interval");
    scheduleAt(inet::simTime() + intervalLength, reminderMessage);
}


void RTCP::readRet(inet::cPacket *sifpIn)
{
...
void RTCP::createSocket()
{
    ...
    if (_socketFdIn == -1) {
        ...
        inet::cMessage *msg = new inet::cMessage("UDP_C_BIND", UDP_C_BIND);
...
void RTCP::scheduleInterval(){
    ...
    inet::simtime_t intervalLength = _averagePacketSize * (inet::simtime_t)(_participantInfos->size()) / (inet::simtime_t)(_bandwidth * _rtcpPercentage * (_senderInfo->isSender() ? 1.0 : 0.75) / 100.0);
...
void RTCP::createPacket()
{
    ...
    if (_senderInfo->isSender()) {
        ...
        senderReportPacket->setSenderReport(_senderInfo->senderReport(inet::simTime()));
    ...
    for (int i = 0; i < _participantInfos->size(); i++) {
        ...
        if (_participantInfos->exist(i)) {
            ...
            if (participantInfo->getSSRC() != _senderInfo->getSSRC()) {
                ReceptionReport *report = ((RTPReceiverInfo *)participantInfo)->receptionReport(inet::simTime());
                if (report != NULL) {
                    reportPacket->addReceptionReport(report);
                }
            }
            participantInfo->nextInterval(inet::simTime());

            if (participantInfo->toBeDeleted(inet::simTime())) {
    ...
    inet::cPacket *msg = new inet::cPacket("RTCPCompoundPacket");
...
{
    _senderInfo->processRTPPacket(packet, getId(), inet::simTime());
}
...
void RTCP::processIncomingRTCPPacket(RTCPCompoundPacket *packet, IPAddress address, int port)
{
    ...
    inet::cArray *rtcpPackets = packet->getRtcpPackets();

    inet::simtime_t arrivalTime = packet->getArrivalTime();
    delete packet;
   ...
   for (int i = 0; i < rtcpPackets->size(); i++) {
        if (rtcpPackets->exist(i)) {
            ...
            if (rtcpPacket->getPacketType() == RTCPPacket::RTCP_PT_SR) {
                ...
                participantInfo->processSenderReport(rtcpSenderReportPacket->getSenderReport(), inet::simTime());

                inet::cArray *receptionReports = rtcpSenderReportPacket->getReceptionReports();
                    ...
                    if (receptionReports->exist(j)) {
                        ...
                        if (_senderInfo) {
                            if (receptionReport->getSSRC() == _senderInfo->getSSRC()) {
                                _senderInfo->processReceptionReport(receptionReport, inet::simTime());
                            }
                        }
                        else
                ...
                inet::cArray *receptionReports = rtcpReceiverReportPacket->getReceptionReports();
                for (int j = 0; j < receptionReports->size(); j++) {
                    if (receptionReports->exist(j)) {
                        ReceptionReport *receptionReport = (ReceptionReport *)(receptionReports->remove(j));
                        if (_senderInfo) {
                ...
                inet::cArray *sdesChunks = rtcpSDESPacket->getSdesChunks();

                for (int j = 0; j < sdesChunks->size(); j++) {
                    if (sdesChunks->exist(j)) {
                        // remove the sdes chunk from the inet::cArray of sdes chunks
