...
void RTPSenderInfo::processRTPPacket(RTPPacket *packet, int id,  inet::simtime_t arrivalTime)
{
    _packetsSent++;
...
void RTPSenderInfo::processReceptionReport(ReceptionReport *report, inet::simtime_t arrivalTime)
{
    delete report;
}


SenderReport *RTPSenderInfo::senderReport(inet::simtime_t now)
{
    if (isSender()) {
...
void RTPSenderInfo::setStartTime(inet::simtime_t startTime)
{
...
bool RTPSenderInfo::toBeDeleted(inet::simtime_t now)
{
    return false;
}
