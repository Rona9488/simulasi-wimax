    ...
    while (true) {
        ...
        if (msg->getArrivalGateId() == findGate("profileIn")) {
            ...
            if (rinpIn->getType() == RTPInnerPacket::RTP_INP_INITIALIZE_SENDER_MODULE) {
                ...
                if (!inet::opp_strcmp(command, "PLAY")) {
                    play();
                }
                else if (!inet::opp_strcmp(command, "PLAY_UNTIL_TIME")) {
                    playUntilTime(rscm->getCommandParameter1());
                }
                else if (!inet::opp_strcmp(command, "PLAY_UNTIL_BYTE")) {
                    playUntilByte(rscm->getCommandParameter1());
                }
                else if (!inet::opp_strcmp(command, "PAUSE")) {
                    pause();
                }
                else if (!inet::opp_strcmp(command, "STOP")) {
                    stop();
                }
                else if (!inet::opp_strcmp(command, "SEEK_TIME")) {
                    seekTime(rscm->getCommandParameter1());
                }
                else if (!inet::opp_strcmp(command, "SEEK_BYTE")) {
...
{
    std::cout << "initializeSenderModule Enter" << std::endl;
    _mtu = rinpIn->getMTU();
    ...
    _status = STOPPED;
    std::cout << "initializeSenderModule Exit" << std::endl;
}
...
void RTPPayloadSender::openSourceFile(const char *fileName)
{
    ...
    if (!_inputFileStream) {
        throw std::runtime_error("sender module: error opening data file");
    }
}
...
void RTPPayloadSender::playUntilTime(inet::simtime_t moment)
{
...
void RTPPayloadSender::seekTime(inet::simtime_t moment)
{
