...
void RTP::handleMessage(inet::cMessage *msg)
{
...
void RTP::handleMessageFromApp(inet::cMessage *msg)
{
...
void RTP::handleMessageFromProfile(inet::cMessage *msg)
{
    ...
    std::cout << "handleMessageFromProfile(inet::cMessage *msg) Exit"<<std::endl;
}


void RTP::handleMessageFromRTCP(inet::cMessage *msg)
{
    RTPInnerPacket *rinp = check_and_cast<RTPInnerPacket *>(msg);
...
void RTP::handleMessagefromUDP(inet::cMessage *msg)
{
    readRet(msg);
}
...
void RTP::createSenderModule(RTPInterfacePacket *rifp)
{
    ...
    std::cout << rifp->getSSRC()<<std::endl;
...
void RTP::readRet(inet::cMessage *sifp)
{
    if ( ! _leaveSession)
    {
...
//    profile->scheduleStart(inet::simTime());
}
...
void RTP::initializeRTCP()
{
    ...
    rinp->initializeRTCP(inet::opp_strdup(_commonName), _mtu, _bandwidth, _rtcpPercentage, _destinationAddress, rtcpPort);
    send(rinp, "rtcpOut");
}
