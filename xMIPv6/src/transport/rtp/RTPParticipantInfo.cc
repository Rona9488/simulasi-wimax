...
void RTPParticipantInfo::processRTPPacket(RTPPacket *packet, int id, inet::simtime_t arrivalTime)
{
    _silentIntervals = 0;
    delete packet;
}
...
void RTPParticipantInfo::processSenderReport(SenderReport *report, inet::simtime_t arrivalTime)
{
...
void RTPParticipantInfo::processReceptionReport(ReceptionReport *report, inet::simtime_t arrivalTime)
{
    // useful code can be found in subclasses
    delete report;
}
...
void RTPParticipantInfo::processSDESChunk(SDESChunk *sdesChunk, inet::simtime_t arrivalTime)
{
...
ReceptionReport *RTPParticipantInfo::receptionReport(inet::simtime_t now)
{
    return NULL;
}


SenderReport *RTPParticipantInfo::senderReport(inet::simtime_t now)
{
    return NULL;
}


void RTPParticipantInfo::nextInterval(inet::simtime_t now)
{
    _silentIntervals++;
}


bool RTPParticipantInfo::toBeDeleted(inet::simtime_t now)
{
    return false;
}
