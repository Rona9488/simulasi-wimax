 ...
 */
class INET_API UDP : public inet::cSimpleModule
{
  public:
    struct SockDesc
    {
  ...
  protected:
    ...
    virtual void sendUp(inet::cPacket *payload, UDPPacket *udpHeader, IPControlInfo *ctrl, SockDesc *sd);
    virtual void sendUp(inet::cPacket *payload, UDPPacket *udpHeader, IPv6ControlInfo *ctrl, SockDesc *sd);
    virtual void processUndeliverablePacket(UDPPacket *udpPacket, inet::cObject *ctrl);
    ...
    virtual void processICMPError(inet::cPacket *icmpErrorMsg); // TODO use ICMPMessage

    // process UDP packets coming from IP
    virtual void processUDPPacket(UDPPacket *udpPacket);

    // process packets from application
    virtual void processMsgFromApp(inet::cPacket *appData);

    // process commands from application
    virtual void processCommandFromApp(inet::cMessage *msg);
