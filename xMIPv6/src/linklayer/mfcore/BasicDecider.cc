...
#define coreEV (coreDebug ? std::cout << logName() << "::BasicDecider: " : std::cout)
 ...
 */
void BasicDecider::handleMessage(inet::cMessage *msg)
{
...
{
    numSentUp++;
    inet::cPacket *macMsg = frame->decapsulate();
    send(macMsg, uppergateOut);
    coreEV << "sending up msg " << frame->getName() << std::endl;
    delete frame;
}
