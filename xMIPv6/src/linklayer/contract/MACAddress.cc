...
{
    if (k>=6) throw inet::cRuntimeError("Array of size 6 indexed with %d", k);
    return address[k];
}
...
{
    if (!tryParse(hexstr))
        throw inet::cRuntimeError("MACAddress: wrong address syntax '%s': 12 hex digits expected, with optional embedded spaces, hyphens or colons", hexstr);
}
...
InterfaceToken MACAddress::formInterfaceIdentifier() const
{
    ...
    uint32_t high = (b[0]<<24) | (b[1]<<16) | (b[2]<<8) | 0xff;
    uint32_t low =  (0xfe<<24) | (b[3]<<16) | (b[4]<<8) | b[5];
