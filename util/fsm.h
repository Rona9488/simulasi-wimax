...
#define TIMER(name, time, descr)                              \
    ...
    Timer##name        = new omnetpp::cMessage(descr);

#define TIMER_START(name)                                     \
    scheduleAt(omnetpp::simTime() + Timer##name##Value, Timer##name);
...
#define STATIC_TIMER_START(obj, name) \
    obj->scheduleAt(omnetpp::simTime() + obj->Timer##name##Value, obj->Timer##name);
...
typedef FsmStateType onEventFunc(Fsm *fsm, FsmEventType e, omnetpp::cMessage *msg);
...
class Fsm : public omnetpp::cSimpleModule {
public:
    Fsm();
    ...
    virtual void onEvent(FsmEventType e, omnetpp::cMessage *msg);
