...
bool Fsm::stateVerify() {
    ...
    if (errorFlag)
        error("%s: Not all states have been inited properly.", getFullName());
    return true;
}
...
bool Fsm::eventVerify() {
    ...
    if (errorFlag)
        error("%s: Not all events have been inited properly.", getFullName());
    return true;
}
void Fsm::onEvent(FsmEventType e, omnetpp::cMessage *msg)
{
    if ( (e<0) || (e>EventsCnt) ) {
        error("%s: Invalid event type %d specified (0..%d allowed).\n", getFullName(), e, StatesCnt);
    }
	...
	error("%s: Invalid state (%d) returned duing %s event processing in state %s.", getFullName(),
...
void Fsm::stateSet(FsmStateType newState)
{
	...
	if (transitionsCnt > FSM_MAX_TRANSITIONS)
	    error("%s: probably state transitions loop detected: %d transitions occured without stationary state.\n",
	...
	if (to->onEnter) {
	    ...
	    error("%s: Invalid state type %d returned in %s::onEnter() (0..%d allowed).\n", getFullName(), tmp,
